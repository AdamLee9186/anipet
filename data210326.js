...
/* ====================================================================
   הסתרת שורות מרלוג (גרסת CSS יציבה - מופעל כברירת מחדל + עיצוב מעודכן)
==================================================================== */
(function initHideMerlogFeature() {
    const TBL_SELECTOR = '#operator-store-visits-table';
    let isHideActive = true; // הפעלה כברירת מחדל
    let tableObserver = null;

    // 1. הזרקת חוק CSS גלובלי להסתרה
    if (!document.getElementById('merlog-hide-style')) {
        const style = document.createElement('style');
        style.id = 'merlog-hide-style';
        style.textContent = `
            body.hide-merlog-active tr.is-merlog-row {
    ...
    // 2. זיהוי מדויק: עוקף את הבאג של התוויות בעמוד 2, ומחפש ישירות את רכיב הנהג
    function isMerlog(tr) {
        if (!tr) return false;

        // Find the native select element for the driver directly (ignores missing data-labels)
        const driverSelect = tr.querySelector('.visit-drivers-select2');
        if (!driverSelect) return false;

        // Primary Check: Read the selected option from the native <select>
        if (driverSelect.options && driverSelect.options.length > 0 && driverSelect.selectedIndex >= 0) {
            const selectedOption = driverSelect.options[driverSelect.selectedIndex];
            if (selectedOption && (selectedOption.text || '').includes('מרלוג')) {
                return true;
            }
        }

        // Secondary Fallback Check: Look at the rendered Select2 text
        const driverContainer = driverSelect.closest('td');
        if (driverContainer) {
            const renderedEl = driverContainer.querySelector('.select2-selection__rendered');
            if (renderedEl && (renderedEl.textContent || '').includes('מרלוג')) {
                return true;
            }
        }
    ...
    // 3. תיוג השורות ב-DOM (עם אופטימיזציית ביצועים קיצונית)
    function tagMerlogRows() {
        const tbody = document.querySelector(`${TBL_SELECTOR} tbody`);
        if (!tbody) return;

        // Performance Fix: Query ONLY rows that haven't been scanned yet
        const unscannedRows = tbody.querySelectorAll('tr:not([data-merlog-scanned="true"])');

        unscannedRows.forEach(tr => {
            if (tr.querySelector('.dataTables_empty')) return;

            // Mark this row as scanned so we never process its DOM again unless it changes
            tr.setAttribute('data-merlog-scanned', 'true');

            // Run the heavy check only once per row
            if (isMerlog(tr)) {
                tr.classList.add('is-merlog-row');
            } else {
                tr.classList.remove('is-merlog-row');
            }

            // Bind a lightweight event listener in case the user changes the driver inline
            const driverSelect = tr.querySelector('.visit-drivers-select2');
            if (driverSelect && !driverSelect.hasAttribute('data-merlog-listener')) {
                driverSelect.setAttribute('data-merlog-listener', 'true');
                // Use jQuery to listen to Select2 changes if available, otherwise native change
                const jq = window.jQuery || window.$;
                if (jq) {
                    jq(driverSelect).on('change.select2', () => {
                        tr.removeAttribute('data-merlog-scanned');
                        tagMerlogRows(); // Re-evaluate this specific row
                    });
                } else {
        ...
        // Fast DOM query to count total hidden rows on the current page
        const hiddenCount = tbody.querySelectorAll('tr.is-merlog-row').length;
        updateCounters(hiddenCount);
    }

    // 4. עדכון מונה חזותי
    function updateCounters(hiddenCount) {
        const header = document.querySelector('.d-flex.align-items-baseline h5.text-dark');
        if (!header) return;

        let badge = document.getElementById('merlog-hidden-badge');
        if (!badge) {
            badge = document.createElement('span');
            badge.id = 'merlog-hidden-badge';
            badge.style.cssText = 'font-size: 14px; font-weight: normal; color: #dc3545; margin-right: 8px;';
            header.appendChild(badge);
        }

        const newText = (isHideActive && hiddenCount > 0) ? `(שורות מרלוג מוסתרות: ${hiddenCount})` : '';
        if (badge.innerText !== newText) {
            badge.innerText = newText;
        }
    }

    // 5. ניהול האזנה לטבלה: מאזין לכל הטבלה כדי לשרוד החלפות עמודים
    function bindTableObserver() {
        const table = document.querySelector(TBL_SELECTOR);
        if (!table) return;
    ...
    // 6. בניית כפתור ה-UI
    function buildUI() {
        const filtersContainer = document.querySelector('#filters-presets-box .d-flex.align-items-center');
        if (!filtersContainer || document.getElementById('hide-merlog-btn')) return false;

        const iconElement = filtersContainer.querySelector('.fa-filter-list');
        ...
        btn.id = 'hide-merlog-btn';
        btn.className = 'filter-badge cursor-pointer mr-2';
        btn.innerText = 'הסתר מרלוג';

        // הוסר העיצוב של ה-Bold לבקשת המשתמש
        btn.style.cssText = `
            align-items: center;
            display: flex;
        ...
        function updateBtnStyle() {
            if (isHideActive) {
                btn.style.backgroundColor = 'rgb(54, 153, 255)';
                btn.style.color = 'white';
                document.body.classList.add('hide-merlog-active');
            } else {
                btn.style.backgroundColor = 'transparent';
                btn.style.color = 'rgb(54, 153, 255)';
                document.body.classList.remove('hide-merlog-active');
            }
        }

        btn.addEventListener('mouseenter', () => {
            if (!isHideActive) btn.style.backgroundColor = 'rgba(54, 153, 255, 0.1)';
        });
        btn.addEventListener('mouseleave', () => {
            if (!isHideActive) btn.style.backgroundColor = 'transparent';
        });
        ...
        iconWrapper.insertAdjacentElement('afterend', btn);
        updateBtnStyle(); // מעדכן את העיצוב הראשוני לפי המצב (דלוק כברירת מחדל)
    ...
    const initObserver = new MutationObserver(() => {
        const hasTable = document.querySelector(`${TBL_SELECTOR} tbody`);
        const hasFilterIcon = document.querySelector('#filters-presets-box .fa-filter-list');

        if (hasTable && hasFilterIcon) {
            if (buildUI()) {
                bindTableObserver();
                tagMerlogRows(); // מריץ את ההסתרה מיד בטעינה הראשונית
    ...
    if (obsRoot) {
        initObserver.observe(obsRoot, { childList: true, subtree: true });
    }
})();
...
function __tmcEnsurePreviewCSS(){
  try{
    ...
    .tmc-preview-card.tmc-bg-picked-full,
    tr.tmc-bg-picked-full,
    tr.tmc-bg-picked-full > td {
      background-color: #eafaf1 !important;
    }
    .tmc-preview-card.tmc-bg-picked-none,
    tr.tmc-bg-picked-none,
    tr.tmc-bg-picked-none > td {
      background-color: #fdeded !important;
    }
    .tmc-preview-card.tmc-bg-picked-partial,
    tr.tmc-bg-picked-partial,
    tr.tmc-bg-picked-partial > td {
    ...
    td[data-tmc-pick-status="full"],
    td[data-tmc-pick-status="full"] * { color: #166534 !important; font-weight: 600; }
    .tampermonkey-picked-partial,
    td[data-tmc-pick-status="partial"],
    td[data-tmc-pick-status="partial"] * { color: #c26a00 !important; font-weight: 600; }
    .tampermonkey-picked-none,
    td[data-tmc-pick-status="none"],
    td[data-tmc-pick-status="none"] * { color: #991b1b !important; font-weight: 600; }
    ...
    // Hedge against late injections: stabilize briefly after first paint (no per-frame rAF loop)
    // We do one delayed pass + a short-lived HEAD observer to dedupe late duplicates.
    let _tmcPreviewCssStabilized = false;
    const stabilizeOnce = () => {
      if (_tmcPreviewCssStabilized) return;
      _tmcPreviewCssStabilized = true;
    ...
    // One delayed pass is typically enough
    setTimeout(stabilizeOnce, 400);

    // If the page injects another style#tmc-preview-css later, catch it once (then stop observing)
    try{
      let t = 0;
      const schedule = () => {
        clearTimeout(t);
        t = setTimeout(() => { _tmcPreviewCssStabilized = false; stabilizeOnce(); }, 150);
      };
      const headMO = new MutationObserver((muts) => {
        for (const m of muts){
          for (const n of m.addedNodes || []){
            if (n && n.nodeType === 1 && n.id === 'tmc-preview-css') { schedule(); return; }
          }
        }
      });
      headMO.observe(document.head, { childList: true });
      setTimeout(() => { try{ headMO.disconnect(); }catch(_){} }, 2500);
    }catch(_){}
  }catch(_){}
}
...
/* =========================
   DATA-LABEL FALLBACK
   When Lionwheel's responsive mode is OFF, <td> elements may not have data-label.
   Many Toolbox features rely on data-label selectors, so we synthesize them from <thead>.
   ========================= */
function tmcNormalizeHeaderText(str) {
    return String(str || '')
        .replace(/\s+/g, ' ')
        .replace(/[‎‏‪-‮]/g, '')
        .trim();
...
function tmcBuildHeaderLabelsByIndex(table) {
    const ths = Array.from(table.querySelectorAll('thead th'));
    if (!ths.length) return [];

    return ths.map(th => {
        // Prefer explicit header text; fallback to aria-label prefix if needed
        const direct = tmcNormalizeHeaderText(th.textContent);
        if (direct) return direct;

        const aria = tmcNormalizeHeaderText(th.getAttribute('aria-label'));
        if (!aria) return '';

        // aria-label often contains: "טלפון: activate to sort..." -> take the part before ':'
        const colonIdx = aria.indexOf(':');
        return colonIdx > 0 ? aria.slice(0, colonIdx).trim() : aria;
    });
}

function tmcEnsureDataLabelsForTable(table) {
    try {
        if (!table) return;

        // If the page already provided data-labels, do nothing
        const alreadyHas = table.querySelector('tbody td[data-label]');
        if (alreadyHas) return;

        const labels = tmcBuildHeaderLabelsByIndex(table);
        ...
        console.warn('[Toolbox] tmcEnsureDataLabelsForTable failed:', e);
    }
}
...
(function canonicalPreviewLayout(){
  ...
  try {
    if (!window.__tmcPreviewCanonMO){
      const enforceThrottled = (typeof debounce === 'function')
        ? debounce(() => enforce(), 250)
        : (function(){ let t=0; return () => { clearTimeout(t); t=setTimeout(() => { t=0; enforce(); }, 250); }; })();
      ...
      const root = document.getElementById('operator-store-visits-table_wrapper')
                || document.querySelector('#task_offcanvas') || document.body;
...
(function() {
  ...
  function __tmcStartTaskPageStripper(){
    ...
    const STRIP = () => {
      ...
      const re = /\bmerlog(?:-row)?-highlight\b/g;
      nodes.forEach(el => {
  ...
  window.addEventListener('popstate', __tmcMarkPageType);
  window.addEventListener('hashchange', __tmcMarkPageType);
  document.addEventListener('tm:dom-updated', __tmcMarkPageType);
...
(function installUnifiedPassiveWrapper(){
  ...
  try {
    // IMPORTANT: patch the PAGE context EventTarget (unsafeWindow) when available
    const W = (typeof unsafeWindow !== 'undefined') ? unsafeWindow : window;
    const PageEventTarget = W.EventTarget;
    const Orig = PageEventTarget && PageEventTarget.prototype && PageEventTarget.prototype.addEventListener;
    if (!Orig) return;
    ...
    function withinPerfectScrollbar(el){
      // Guard: window/document/non-elements should never be treated as "within PS"
      try{
        if (!el) return false;
        const W = (typeof unsafeWindow !== 'undefined') ? unsafeWindow : window;
        if (el === W || el === W.document) return false;
        // nodeType 1 = Element
        if (el.nodeType !== 1 || typeof el.closest !== 'function') return false;
      }catch(_){ return false; }
    ...
    function isSafeRoot(t){
      try{
        ...
        return (
          ...
          t === W.document.body
        ) || (isElement(t) && matchesAny(t, SAFE_SELECTORS));
      }catch(_){
        return false;
      }
    }
    ...
    PageEventTarget.prototype.addEventListener = function(type, listener, options){
      const t = String(type);
...
function __tmcClampPreviewRowWidth(el) {
  // NO-OP: Deprecated. Replacing JS calculations with CSS relative units.
  if (el) el.style.maxWidth = 'calc(100vw - 100px)';
}

function __tmcClampAllPreviewRows(ctx=document){
  // STOP: This function causes Forced Reflow.
  return;
  if (!__tmcViewportRO){
...
function __tmcColorizePreviewCards(root = document){
  try{
    const scope = root?.querySelectorAll ? root : document;
    const cards = scope.matches?.('.tmc-preview-card')
      ? [scope]
      : Array.from(scope.querySelectorAll('.tmc-preview-card'));

    cards.forEach(card => {
      if (!card || card.nodeType !== 1) return;

      card.classList.remove('tmc-bg-picked-full', 'tmc-bg-picked-none', 'tmc-bg-picked-partial', 'bg-white');
      delete card.dataset.tmcPickStatus;

      const raw = String(card.textContent || '').replace(/\s+/g, ' ').trim();
      const pickState = __tmcGetPickState(raw);
      if (!pickState) return;

      card.classList.add(pickState.className);
      card.dataset.tmcPickStatus = pickState.status;
    });
  }catch(_){}
}

function __tmcGetPickState(text){
  const match = String(text || '').match(/(\d+)\s*\/\s*(\d+)/);
  if (!match) return null;

  const picked = Number(match[1]);
  const total = Number(match[2]);
  if (!Number.isFinite(picked) || !Number.isFinite(total) || total <= 0) return null;

  if (picked === total) {
    return { status: 'full', className: 'tmc-bg-picked-full', picked, total };
  }
  if (picked === 0) {
    return { status: 'none', className: 'tmc-bg-picked-none', picked, total };
  }
  if (picked > 0 && picked < total) {
    return { status: 'partial', className: 'tmc-bg-picked-partial', picked, total };
  }
  return null;
}

function __tmcColorizeTableRows(root = document){
  try{
    const scope = root?.querySelectorAll ? root : document;
    const rows = scope.matches?.('table.table tbody tr')
      ? [scope]
      : Array.from(scope.querySelectorAll('table.table tbody tr'));

    rows.forEach(row => {
      if (!row || row.nodeType !== 1) return;
      if ((row.id || '').startsWith('preview-for-')) return;

      row.classList.remove('tmc-bg-picked-full', 'tmc-bg-picked-none', 'tmc-bg-picked-partial');
      delete row.dataset.tmcPickStatus;
      row.querySelectorAll('td[data-tmc-pick-status]').forEach(td => {
        try { delete td.dataset.tmcPickStatus; } catch(_){}
      });

      const qtyCell = Array.from(
        row.querySelectorAll(
          'td[data-label="כמות / לוקט"], td[data-label*="כמות"], td[data-label*="לוקט"], td[title*="כמות"], td[title*="לוקט"], td[dir="ltr"].align-center'
        )
      ).find(td => /^\s*\d+\s*\/\s*\d+\s*$/.test(td.textContent || ''));
      if (!qtyCell) return;

      const pickState = __tmcGetPickState(qtyCell.textContent || '');
      if (!pickState) return;

      row.classList.add(pickState.className);
      row.dataset.tmcPickStatus = pickState.status;
      qtyCell.dataset.tmcPickStatus = pickState.status;
    });
  }catch(_){}
}
...
function splitPreviewMetaLines(rootEl){
  ...
  metas.forEach(meta => {
    ...
    try { if (typeof highlightPickQuantities === 'function') highlightPickQuantities(meta); }catch(_){ }
    try { __tmcColorizePreviewCards(card || meta); } catch(_){}
...
function fixShipmentWrapping() {
  // NO-OP: Deprecated in favor of CSS rule. 
  // Function kept to prevent "ReferenceError" in legacy calls.
}
...
(function() {
    ...
    // ---< Supplier Index (shared with MissingTable script) >---
    const PRODUCTS_JSON_URL = "https://raw.githubusercontent.com/AdamLee9186/anipet/main/products_optimized.json";

    // Shared supplier index (prefer MissingTable's in-page shared cache)
    const LW_SHARED_NS = '__LW_SHARED';
    const LW_SHARED_SUPPLIER_KEY = 'supplier_index_v1';
    const LW_SHARED_SESSION_SUPPLIER_KEY = 'lw_shared_supplier_index_v1';

    function lwGetSharedRoot() {
      const w = window;
      w[LW_SHARED_NS] = w[LW_SHARED_NS] || {};
      return w[LW_SHARED_NS];
    }

    function lwGetSharedSupplierIndex() {
      const root = lwGetSharedRoot();
      return root[LW_SHARED_SUPPLIER_KEY] || null;
    }

    function lwPublishSharedSupplierIndex(indexObj, { persistSession = false } = {}) {
      const root = lwGetSharedRoot();
      root[LW_SHARED_SUPPLIER_KEY] = indexObj;
      if (!persistSession) return;
      try {
        sessionStorage.setItem(LW_SHARED_SESSION_SUPPLIER_KEY, JSON.stringify(indexObj));
      } catch (e) {}
    }

    function lwBuildCompactSupplierIndex(data) {
      const suppliers = (data && data.metadata && Array.isArray(data.metadata.suppliers_list))
        ? data.metadata.suppliers_list.slice()
        : [];
      const supplierToId = new Map();
      suppliers.forEach((s, i) => supplierToId.set(String(s), i));
      ...
      for (const p of products) {
        const sup = (p && p.supplier) ? String(p.supplier) : '';
        if (!sup) continue;
        let sid = supplierToId.get(sup);
        if (sid === undefined) {
          sid = suppliers.length;
          suppliers.push(sup);
          supplierToId.set(sup, sid);
        }

        const barcode = (p && p.barcode) ? String(p.barcode).trim() : '';
        const sku = (p && p.sku) ? String(p.sku).trim() : '';

        if (barcode) byBarcode[barcode] = sid;
        if (sku) bySku[sku] = sid;
      }

      return { v: 1, createdAt: Date.now(), suppliers, byBarcode, bySku };
    }

    let supplierIndexPromise = null;
    async function lwEnsureSupplierIndex() {
      const shared = lwGetSharedSupplierIndex();
      if (shared && shared.byBarcode && shared.suppliers) return shared;

      // try sessionStorage (optional)
      try {
        const raw = sessionStorage.getItem(LW_SHARED_SESSION_SUPPLIER_KEY);
        if (raw) {
          const parsed = JSON.parse(raw);
          if (parsed && parsed.byBarcode && parsed.suppliers) {
            lwPublishSharedSupplierIndex(parsed, { persistSession: false });
            return parsed;
          }
        }
      } catch (e) {}

      // load once (no localStorage / GM_setValue)
      if (!supplierIndexPromise) {
        supplierIndexPromise = (async () => {
          const res = await fetch(PRODUCTS_JSON_URL, { cache: 'no-store' });
          const data = await res.json();
          const idx = lwBuildCompactSupplierIndex(data);
          lwPublishSharedSupplierIndex(idx, { persistSession: false });
          return idx;
        })().catch((e) => {
          supplierIndexPromise = null;
          throw e;
        });
      }
      return supplierIndexPromise;
    }

    async function lookupSupplier(barcode, sku) {
      const b = (barcode ? String(barcode).trim() : '');
      const s = (sku ? String(sku).trim() : '');
      if (!b && !s) return '';

      try {
        const idx = await lwEnsureSupplierIndex();
        if (idx && idx.suppliers) {
          if (b && idx.byBarcode && idx.byBarcode[b] !== undefined) {
            return idx.suppliers[idx.byBarcode[b]] || '';
          }
          if (s && idx.bySku && idx.bySku[s] !== undefined) {
            return idx.suppliers[idx.bySku[s]] || '';
          }
        }
      } catch (e) {}
      return '';
    ...
    const TM_PREVIEW = (() => {
      ...
      const prefetchQueue = new Set(); // Track active prefetches to avoid duplicates
      ...
      async function fetchPanelView(taskId, { force = false, isPrefetch = false } = {}) {
        ...
        if (!force && backoffUntil && now() < backoffUntil) {
          ...
          if (c) return isPrefetch ? null : c.html; // serve stale
      ...
      async function renderPreviewHTML(targetEl, html) {
        ...
        // FAST RENDER: No animations, no requestAnimationFrame delays.
        // Direct DOM injection to eliminate "white space" lag.
        targetCell.innerHTML = `<div class="tm-preview-container" data-preview-rendered="true">${content}</div>`;
        
        // Init tabs immediately (microtask) - only if function exists
        queueMicrotask(() => {
            const wrapper = targetCell.firstElementChild;
            if (wrapper && typeof __tmcInitTabsInPreview === 'function') {
                __tmcInitTabsInPreview(wrapper);
            }
        });
      }
    ...
    let productSkuMap = null; // Optimized Map for O(1) lookup
    let productNameMap = null; // Optimized Map for O(1) lookup
    ...
    // Helper to build optimized maps from the raw array
    function buildProductMaps(data) {
        try {
            ...
            for (const item of data) {
                // Map all SKUs to this item
                if (item.skus && Array.isArray(item.skus)) {
                    for (const sku of item.skus) {
                        productSkuMap.set(sku, item);
                    }
                }
                // Map product name
                if (item.productName) {
                    productNameMap.set(item.productName.toLowerCase().trim(), item);
                }
            }
            console.log(`[${SCRIPT_NAME}] Optimized maps built: ${productSkuMap.size} SKUs, ${productNameMap.size} Names`);
        } catch (e) {
            console.error(`[${SCRIPT_NAME}] Error building optimized maps:`, e);
        }
    }
        ...
        try {
            ...
            if (cachedData && (Date.now() - cachedTimestamp < CACHE_DURATION_MS)) {
                ...
                if (decompressed) {
                    ...
                    buildProductMaps(productDataCache); // Build maps immediately after loading cache
            ...
            buildProductMaps(productDataCache); // Build maps immediately after parsing CSV
...
function showGalleryOverlay(galleryItems, startIndex) {
    ...
    // Prevent browser scrolling/zoom gestures so our touch handlers can be passive
    ...
    productInfoElement.style.marginBottom = '0px';

    // Meta row (icons instead of "ספק/מק\"ט/מחיר") - vertical layout, centered
    ...
    metaRow.style.color = 'rgba(255,255,255,0.85)';
    metaRow.style.fontSize = '14px';
    metaRow.style.margin = '4px 0 2px';
    productInfoElement.appendChild(metaRow);

    // Order: 1. Barcode, 2. Price, 3. Supplier
    const skuElement = document.createElement('span');
    skuElement.className = 'gallery-meta-barcode';
    skuElement.style.display = 'inline-flex';
    skuElement.style.alignItems = 'center';
    skuElement.style.gap = '6px';
    const barcodeIcon = document.createElement('i');
    barcodeIcon.className = 'fa-light fa-barcode';
    barcodeIcon.style.opacity = '0.9';
    barcodeIcon.title = 'ברקוד';
    skuElement.appendChild(barcodeIcon);
    const skuTextSpan = document.createElement('span');
    skuTextSpan.className = 'gallery-meta-text';
    skuElement.appendChild(skuTextSpan);
    metaRow.appendChild(skuElement);

    const priceElement = document.createElement('span');
    priceElement.className = 'gallery-meta-price';
    priceElement.style.display = 'none'; // toggled if price exists
    metaRow.appendChild(priceElement);

    const supplierElement = document.createElement('span');
    supplierElement.className = 'gallery-meta-supplier';
    supplierElement.style.display = 'none';
    supplierElement.style.alignItems = 'center';
    supplierElement.style.gap = '6px';
    supplierElement.style.whiteSpace = 'nowrap';
    // icon + text span (text is set via textContent to avoid HTML injection)
    const supplierIcon = document.createElement('i');
    supplierIcon.className = 'fa-light fa-truck';
    supplierIcon.style.opacity = '0.9';
    supplierIcon.title = 'ספק';
    supplierElement.appendChild(supplierIcon);
    const supplierTextSpan = document.createElement('span');
    supplierTextSpan.className = 'gallery-meta-text';
    supplierElement.appendChild(supplierTextSpan);
    metaRow.appendChild(supplierElement);
    ...
    const updateGalleryView = () => {
        ...
        // Build: [barcode icon] + [wrap(bdi+copy)]  (no "מק\"ט:" label)
        ...
        skuTextSpan.textContent = '';
        skuTextSpan.appendChild(skuWrap);

        // Supplier (prefer shared index from MissingTable; otherwise load once in-memory)
        supplierTextSpan.textContent = '';
        supplierElement.style.display = 'none';
        const _supplierSeq = ++supplierLookupSeq;
        lookupSupplier(skuBdi.textContent, originalSku).then((supplier) => {
            if (_supplierSeq !== supplierLookupSeq) return; // stale
            supplierTextSpan.textContent = supplier || '';
            supplierElement.style.display = supplier ? 'inline-flex' : 'none';
        }).catch(() => {
            if (_supplierSeq !== supplierLookupSeq) return;
            supplierTextSpan.textContent = '';
            supplierElement.style.display = 'none';
        });
        ...
        // Add price if available (icon instead of "מחיר:")
        if (item.price) {
            priceElement.innerHTML = `<i class="fa-light fa-tag" title="מחיר" style="opacity: 0.9;"></i> <strong class="barcode-highlight-gallery copy-enabled" title="לחץ להעתקה">₪${item.price}</strong>`;
            priceElement.style.display = 'inline-flex';
            priceElement.style.alignItems = 'center';
            priceElement.style.gap = '6px';
        } else {
    ...
    // Add click event listener for copy functionality in gallery
    overlay.addEventListener('click', (e) => {
        try {
            const raw = e.composedPath ? e.composedPath()[0] : e.target;
            const targetEl = raw && raw.nodeType === 1 ? raw : null;
            if (!targetEl) return;

            // Handle copy-icon clicks (for barcode and name)
            const copyIcon = targetEl.closest('.copy-icon');
            if (copyIcon) {
                e.preventDefault();
                e.stopPropagation();
                // Find the text to copy from the parent tampermonkey-copy-wrap
                const wrap = copyIcon.closest('.tampermonkey-copy-wrap');
                if (wrap) {
                    const bdi = wrap.querySelector('.gallery-barcode-bdi, .gallery-name-bdi');
                    if (bdi) {
                        const textToCopy = bdi.textContent.trim();
                        if (textToCopy) {
                            navigator.clipboard.writeText(textToCopy).then(() => {
                                tmToast('הועתק!', copyIcon);
                            }).catch(console.warn);
                            return;
                        }
                    }
            ...
            // Handle copy-enabled elements (like price) - check before closest to avoid double handling
            if (targetEl.classList.contains('copy-enabled')) {
                e.preventDefault();
                e.stopPropagation();
                const textToCopy = targetEl.textContent.trim();
                if (textToCopy) {
                    copyWithFeedback(targetEl, textToCopy);
                    return;
                }
            }

            // Handle copy-enabled elements inside other elements (for nested cases)
            const copyEnabledParent = targetEl.closest('.copy-enabled');
            if (copyEnabledParent && copyEnabledParent !== targetEl && !targetEl.closest('.copy-icon')) {
                e.preventDefault();
                e.stopPropagation();
                const textToCopy = copyEnabledParent.textContent.trim();
                if (textToCopy) {
                    copyWithFeedback(copyEnabledParent, textToCopy);
                    return;
                }
            }

            // Handle direct clicks on gallery meta text (barcode/meta text without copy-icon)
            if (targetEl.closest('.gallery-meta-barcode, .gallery-meta-price')) {
                const metaElement = targetEl.closest('.gallery-meta-barcode, .gallery-meta-price');
                // Only handle if not already handled by copy-icon or copy-enabled
                if (!targetEl.closest('.copy-icon') && !targetEl.classList.contains('copy-enabled')) {
                    const textElement = metaElement.querySelector('.gallery-meta-text, .barcode-highlight-gallery');
                    if (textElement && textElement.contains(targetEl)) {
                        e.preventDefault();
                        e.stopPropagation();
                        const textToCopy = textElement.textContent.trim();
                        if (textToCopy) {
                            copyWithFeedback(textElement, textToCopy);
                            return;
                        }
                    }
                }
            ...
            console.warn('Error handling gallery copy:', error);
        }
    }, { passive: false });
            ...
            searchScope.querySelectorAll(searchSelector).forEach(row => {
                ...
                if (match && match.image) {
                    ...
                    items.push({
                        ...
                        quantity: quantity || '',
                        price: price,
                        link: match ? (match.link || null) : null // Add product link if available even without image
                    });
                    uniqueSkus.add(normalizedSku);
                }
            });
    ...
    function injectPreviewFunctionality(mainTableBody) {
        ...
        mainTableBody.querySelectorAll('tr').forEach(row => {
            ...
            button.addEventListener('click', async (e) => {
                ...
                // Trigger immediate fetch (will use prefetch promise if exists)
                ...
                if (taskId) TM_PREVIEW.prefetch(taskId);
                if (existingPreview) {
                ...
                try {
                        ...
                        if (skuIndex !== -1 && nameIndex !== -1 && quantityIndex !== -1) {
                            productTable.querySelectorAll('tbody tr').forEach(itemRow => {
                                ...
                                allItems.push({
                                    ...
                                    link: imageMatch ? (imageMatch.link || null) : null
                                });
                            });
                        }
                    }
                            ...
                            img.onclick = () => {
                                ...
                                const galleryData = allItems.map(i => {
                                    return {
                                        ...
                                        quantity: i.quantity,
                                        price: i.price,
                                        link: i.link || null // Use product link from CSV if available
                                    };
                                });
                    ...
                    try { __tmcBootstrapPreviews(newRow); } catch(_){}
                    try { __tmcColorizePreviewCards(newRow); } catch(_){}
        ...
        // Wire prefetch specifically to new rows
        mainTableBody.querySelectorAll('tr[data-task-id]:not([data-prefetch-wired])').forEach(tr => {
            TM_PREVIEW.wireHoverPrefetch(tr);
            tr.setAttribute('data-prefetch-wired', 'true');
        });
...
function initializeSidePanelResizeObserver() {
  try {
    (function () {
      ...
      // Debounced scheduler: many observers/events may fire in bursts
      const scheduleApplyGapVars = (typeof debounce === 'function')
        ? debounce(applyGapVars, 120)
        : (function(){ let t=0; return () => { clearTimeout(t); t=setTimeout(applyGapVars, 120); }; })();

      // Initial apply
      applyGapVars();

      // Also apply after a short delay to ensure all elements are loaded
      setTimeout(scheduleApplyGapVars, 500);

      // Apply again after DOM is fully loaded
      if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => {
          setTimeout(scheduleApplyGapVars, 100);
      ...
      window.addEventListener('resize', scheduleApplyGapVars, { passive: true });

      // Also re-apply when screen orientation changes
      window.addEventListener('orientationchange', () => {
        setTimeout(scheduleApplyGapVars, 100); // Small delay to ensure orientation change is complete
      });
      ...
      if (panel) {
        const ro = new ResizeObserver(() => scheduleApplyGapVars());
        ro.observe(panel);
      }

      // Optional: if the preview rows are injected later, re-apply when they appear
      const mo = new MutationObserver((muts) => {
        // Only react to relevant late injections (preview rows / map container), not every subtree change.
        for (const m of muts){
          if (m.type !== 'childList') continue;
          for (const n of m.addedNodes || []){
            if (n && n.nodeType === 1){
              if (n.matches?.('tr[id^="preview-for-"], #desktop-map-container') ||
                  n.querySelector?.('tr[id^="preview-for-"], #desktop-map-container')) {
                scheduleApplyGapVars();
                return;
              }
            }
          }
        ...
        mutations.forEach((mutation) => {
          if (mutation.type === 'attributes' &&
            ...
            if (target.matches && target.matches('.offcanvas, .offcanvas-right, .offcanvas-custom')) {
              scheduleApplyGapVars(); // Debounced (replaces timer storm)
            }
          }
        });
      });
      ...
      document.addEventListener('click', (e) => {
        ...
        if (target.matches && (
          ...
          setTimeout(scheduleApplyGapVars, 100); // Delay to allow offcanvas to open/close
        }
      });

      // Monitor for offcanvas events
      document.addEventListener('shown.bs.offcanvas', scheduleApplyGapVars);
      document.addEventListener('hidden.bs.offcanvas', scheduleApplyGapVars);
      ...
      // Periodic check (fallback): only when ResizeObserver is unavailable or panel is missing.
      if (!('ResizeObserver' in window) || !panel) {
        ...
        (function startPeriodicCheck(){
          debouncedPanelCheck();
          if (++checks >= 10) return; // stop after ~20s
...
/* ===== CSS FIX: prevent shipment number wrapping (Replaces JS) ===== */
td[data-label="משלוח"], 
td[data-label="משלוח"] *,
.shipment-no-wrap {
    contain: content; /* CSS Optimization: Tells browser this area won't affect layout outside */
    white-space: nowrap !important;
    word-wrap: normal !important;
    word-break: normal !important;
}

/* New CSS-only Clamping to replace the heavy JS */
.tmc-preview-row, tr[id^="preview-for-"] td[colspan] > div:first-child {
    max-width: calc(100vw - 120px) !important;
    overflow-x: auto !important;
    display: block !important;
}

/* Optimize heavy table renders */
table { contain: size layout style; }
tr { contain: layout style; }
...
tr.merlog-row-highlight .tmc-merlog-cancel:hover {
    filter: brightness(0.95);
}

tr.merlog-row-highlight .tmc-merlog-cancel-hot {
...
function prepareCopyElements() {
    ...
    const merlogRowCache = new Map();
    const MERLOG_CACHE_TTL_MS = 20 * 60e3;

    function cacheGet(taskId) {
        const entry = merlogRowCache.get(taskId);
        if (!entry) return null;
        if ((Date.now() - entry.ts) > MERLOG_CACHE_TTL_MS) {
            merlogRowCache.delete(taskId);
            return null;
        }
        return entry;
    }
    ...
    function cacheSet(taskId, color, source) {
        if (!taskId) return;
        merlogRowCache.set(taskId, { color, source, ts: Date.now() });
    }
    ...
    function highlightMerlogRows() {
        try {
            ...
            table.querySelectorAll('tbody tr').forEach((row) => {
                ...
                const isMerlogArea = rowText.includes('מרלוג צור יגאל');
                ...
                if (isMerlogArea) {
                    ...
                    if (taskId) cacheSet(taskId, 'red', 'dom');
                } else if (taskId) {
    ...
    function highlightMerlogPanelView() {
      try {
        ...
        let merlogFound = merlogPatterns.some(p => panelText.includes(p));
        ...
        const isHighlighted = panelView.dataset.tmcMerlogHighlight === '1';
        if (merlogFound === isHighlighted) return;
        panelView.classList.toggle('merlog-highlight', merlogFound);
        panelView.dataset.tmcMerlogHighlight = merlogFound ? '1' : '0';
      } catch (e) {
    ...
    async function runMainLogic() {
        ...
        safeExecute(async () => {
            ...
            __tmcColorizeTableRows(document);
...
function addClickableLinksToAllTables(force = false, root = document) {
    const __tmcScope = root || document;
    try {
        // OPTIMIZATION: Don't scan unless we have unchecked rows
        // Batch check: Only run if we actually have unprocessed rows in the visible viewport area roughly
        // This prevents the "Forced Reflow" scan on every scroll pixel
        if (!force && !__tmcScope.querySelector('table:not(.pick-order-item-table) tbody tr:not([data-tm-links-done])')) return;
...
// Give late-rendered cells a second and third pass (without rAF "handler took ..." spam)
(function __tmcBootstrapTableFixes(){
  const IS_STORE_VISITS_PAGE = /\/operator\/store_visits/.test(location.pathname);
  const run = () => {
    try{
      if (!IS_STORE_VISITS_PAGE) addClickableLinksToAllTables();
      const panel = document.querySelector('.offcanvas.show, .offcanvas-right.show, .offcanvas-custom.show, #panel_view');
      if (panel){
        if (!IS_STORE_VISITS_PAGE) addClickableLinksToAllTables(false, panel);
        if (settings && settings.replaceBarcodes) replaceBarcodesInViews(panel);
      }
    }catch(_){}
  };

  // Two limited passes only (no per-frame loops)
  setTimeout(run, 250);
  setTimeout(run, 1200);
})();
...
(function observeTablesEverywhere(){
  ...
  // Use a longer debounce for the main engine to stop the "Ticking" feel
  const processAllTableChanges = debounce((mutations) => {
    ...
    try {
      ...
      // IMPORTANT: do NOT run addClickableLinksToAllTables() from the store-visits table processor.
      // It scans other tables and creates unnecessary work on store_visits pages.
    } catch (e) {
      console.warn('[Toolbox] Error in unified table processor:', e);
    }
  }, 500);
  // Debounced schedulers (avoid requestAnimationFrame "handler took ..." violations)
  const IS_STORE_VISITS_PAGE = /\/operator\/store_visits/.test(location.pathname);
  const scheduleGeneral = debounce(() => {
    try{
      // Store-visits page: avoid global table scans; table-specific observer handles phones.
      if (IS_STORE_VISITS_PAGE) return;
      addClickableLinksToAllTables();
    }catch(_){ }
  }, 300);

  const schedulePanel = debounce((panel) => {
    try{
      if (!panel) return;
      // Scope work to the offcanvas itself to avoid scanning the whole document.
      addClickableLinksToAllTables(false, panel);
      if (settings && settings.replaceBarcodes) replaceBarcodesInViews(panel);
    }catch(_){ }
  }, 300);
  ...
  const mo = new MutationObserver(muts => {
    ...
    // Process general document changes (debounced, no rAF)
    if (touched) {
      scheduleGeneral();
      const panel = document.querySelector('.offcanvas.show, .offcanvas-right.show, .offcanvas-custom.show, #panel_view');
  ...
  if (document.body) {
    mo.observe(document.body, {
      // Keep the observer cheap: only structural changes.
      // characterData/attributes on <body> generates a very high mutation rate and causes "ticking".
      childList: true,
      subtree: true
    });
  }
  ...
  document.addEventListener('shown.bs.offcanvas', () => {
    const panel = document.querySelector('.offcanvas.show, .offcanvas-right.show, .offcanvas-custom.show, #panel_view');
    if (panel){
      // Debounced scheduling to avoid heavy work on the same frame the offcanvas opens
      schedulePanel(panel);
    }
  });
})();
...
// Dummy functions to prevent ReferenceError (exposed on window below)
...
// Run once and keep in sync (debounced + mutation-filtered to avoid rAF spam)
renameMakotHeaderToBarcode(document);
const _scheduleRenameHeaders = (typeof debounce === 'function')
  ? debounce(() => renameMakotHeaderToBarcode(document), 400)
  : (function(){ let t=0; return () => { clearTimeout(t); t=setTimeout(() => renameMakotHeaderToBarcode(document), 400); }; })();
const _headersMO = new MutationObserver((muts) => {
  for (const m of muts){
    if (m.type !== 'childList') continue;
    for (const n of m.addedNodes || []){
      if (n && n.nodeType === 1){
        if (n.matches?.('table, thead, th, [data-label]') || n.querySelector?.('table, thead, th, [data-label]')) {
          _scheduleRenameHeaders();
          return;
        }
      }
    }
...
// Run now and keep in sync (debounced + mutation-filtered)
enhanceTablesBarcodeCopyIcons(document);
const _scheduleEnhanceTables = (typeof debounce === 'function')
  ? debounce(() => enhanceTablesBarcodeCopyIcons(document), 450)
  : (function(){ let t=0; return () => { clearTimeout(t); t=setTimeout(() => enhanceTablesBarcodeCopyIcons(document), 450); }; })();
const _tablesMO = new MutationObserver((muts) => {
  for (const m of muts){
    if (m.type !== 'childList') continue;
    for (const n of m.addedNodes || []){
      if (n && n.nodeType === 1){
        // Only react when tables/rows relevant to barcode columns appear
        if (n.matches?.('table, tr, thead, tbody') || n.querySelector?.('table, tr, thead, tbody')) {
          _scheduleEnhanceTables();
          return;
        }
      }
    }
