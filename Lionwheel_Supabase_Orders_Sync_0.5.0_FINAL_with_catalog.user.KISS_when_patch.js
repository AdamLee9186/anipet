  ...
  async function buildStoreVisitsRawRecords(silent = false) {
    ...
    async function syncStoreVisitsIndexToSupabase({
      ...
      const taskIdsAll = Array.from(new Set(rows.map((r) => String(r.task_id || '').trim()).filter(Boolean)));
        ...
        const { data: existingVisits } = await supabaseGetInChunks(
          SUPABASE_VISITS_TABLE, 'task_id', 'task_id', taskIdsAll, IN_CHUNK
        );
        const existingVisitIds = new Set((existingVisits || []).map((r) => String(r.task_id || '')));
      ...
      if (alsoSyncTasks && taskIds.length) {
        const { data: existingOrders } = await supabaseGetInChunks(
          SUPABASE_TABLE, 'order_id', 'order_id', taskIds, IN_CHUNK
        );
        existingOrderIds = new Set((existingOrders || []).map((r) => String(r.order_id || '')));

        const { data: existingItems } = await supabaseGetInChunks(
          SUPABASE_ITEMS_TABLE, 'order_id,line_no', 'order_id', taskIds, IN_CHUNK
        );
        ...
        const lines = [
          ...
          'ביקורים (store_visits):   חדשים ' + stats.visits.new + ' | עודכנו ' + stats.visits.updated + ' | סה"כ נשלחו ' + stats.visits.total,
        ];
        if (alsoSyncTasks) {
          lines.push('משימות (הזמנות):        חדשות ' + stats.tasks.new + ' | עודכנו ' + stats.tasks.updated + ' | סה"כ ' + stats.tasks.total + (fail ? ' | נכשלו ' + fail : ''));
          lines.push('פריטים (order_items):   חדשים ' + stats.items.new + ' | עודכנו ' + stats.items.updated + ' | סה"כ upsert ' + stats.items.total);
          lines.push('—— סיכום: ' + stats.visits.total + ' ביקורים, ' + stats.tasks.total + ' משימות, ' + stats.items.total + ' פריטים נשלחו. חדש: ' + stats.visits.new + '+' + stats.tasks.new + '+' + stats.items.new + ' | עודכן: ' + stats.visits.updated + '+' + stats.tasks.updated + '+' + stats.items.updated + (fail ? ' | נכשלו: ' + fail : '') + ' ——');
        } else {
          lines.push('—— סיכום: ' + stats.visits.total + ' ביקורים נשלחו (חדש ' + stats.visits.new + ', עודכן ' + stats.visits.updated + ') ——');
        }
      ...
      try { localStorage.setItem('lw_store_visits_last_sync_ts', String(Date.now())); } catch (_) {}
      showToast(msg, fail ? 'warning' : 'success');
        ...
        // MV refresh: always at end when we did work or manual force (throttle 10 min unless mvRefreshForce)
        if (didWork || mvRefreshForce) {
          try {
            const mvRes = await maybeRefreshMV({
    ...
    async function maybeRefreshMV({ reason = 'unspecified', force = false, toast = true } = {}) {
      ...
      try {
        ...
        const after = await verifyMVChangeAfterRefresh();
    ...
    /**
     * Compute forecast status from a row from v_forecast_api
     * Calculates n_due, n_soon, n_open, status_tag based on next_due_date
     */
    ...
    /**
     * Fetch forecast data from v_forecast_api
     */
    async function fetchForecastDataByProduct(dateFilter = null) {
      ...
      // Build URL
      let url = `/rest/v1/v_forecast_api?select=*`;

      // Add date filter if provided
      if (dateFilter && dateFilter.from && dateFilter.to) {
        url += `&next_due_date=gte.${dateFilter.from}&next_due_date=lte.${dateFilter.to}`;
      }

      // Order by next_due_date (closest first)
      url += `&order=next_due_date.asc&limit=5000`;
    ...
    async function tmcComputeWhenSummaryForSku(sku, state, row = null) {
      ...
      const cached = FORECAST_WHEN_SUMMARY_CACHE.get(cacheKey);
      if (cached && (nowTs - cached.ts) < FORECAST_WHEN_SUMMARY_TTL_MS) {
        return cached.value || { summary: '', qty: null };
      }

      // Pull per-customer rows and derive unique next_expected_date days
      let rows = [];
      try {
        const aliases = [];
      ...
      const hasOverdue = active.some(r => {
        const d = r.next_expected_date ? new Date(r.next_expected_date) : null;
        if (!d || Number.isNaN(d.getTime())) return false;
        d.setHours(0, 0, 0, 0);
        return d < today;
      });

      // Filter by planning window (dateRangeKey/dateFrom/dateTo) if present
      const range = getPlanningRange(state);
      const inRange = (iso) => {
      ...
      const summaryOvd = tmcSummarizeWhenDates(overdueRecentIsoDates, 3) || (hasOverdue ? 'חריגים' : '');
      const summary = [summaryOvd, summaryUpc].filter(Boolean).join(', ') || '';

      // Qty is derived from the same per-customer rows (consistent with "מתי צפוי")
      const qty = (active || []).reduce((acc, r) => {
        const d = r.next_expected_date ? new Date(r.next_expected_date) : null;
        if (!d || Number.isNaN(d.getTime())) return acc;
        d.setHours(0, 0, 0, 0);
        if (range && (d < range.from || d > range.to)) return acc;
      ...
      FORECAST_WHEN_SUMMARY_CACHE.set(cacheKey, { ts: nowTs, value });
      return value;
    }
      ...
      const rpcPath = '/rest/v1/rpc/forecast_product_summary_by_range_active';
      const allData = [];
      try {
        for (let i = 0; i < limited.length; i += SUMMARY_RPC_BATCH) {
          ...
          const data = await supaRestFetch(rpcPath, {
            method: 'POST',
            body: { p_date_from: dateFrom, p_date_to: dateTo, p_skus: chunk.length ? chunk : null }
          });
      ...
      for (const row of data) {
        ...
        const upcomingIso = (upc || []).map(d => (typeof d === 'string' ? d : (d && d.toISOString ? d.toISOString().slice(0, 10) : null))).filter(Boolean);
        const overdueIso = (ovd || []).map(d => (typeof d === 'string' ? d : (d && d.toISOString ? d.toISOString().slice(0, 10) : null))).filter(Boolean);
        const summaryUpc = tmcSummarizeWhenDates(tmcUniqueSortedIsoDates(upcomingIso), 3);
        const summaryOvd = tmcSummarizeWhenDates(tmcUniqueSortedIsoDates(overdueIso), 3);
        const summary = [summaryOvd, summaryUpc].filter(Boolean).join(', ') || '';
        const allDates = [...overdueIso, ...upcomingIso].filter(Boolean);
        const earliestIso = allDates.length ? allDates.sort()[0] : null;
        let urgencyTier = 0;
        if (earliestIso) {
          const d = new Date(earliestIso);
          d.setHours(0, 0, 0, 0);
        ...
        const priorityScore = urgencyTier * 50 + qtyNum;
        const overdueDates = tmcUniqueSortedIsoDates(overdueIso);
        const upcomingDates = tmcUniqueSortedIsoDates(upcomingIso);
        const entry = { summary, qty: qtyNum ? qtyNum : null, earliestDate: earliestIso, priorityScore, overdueDates, upcomingDates, nOrdersSum, nForecasts };
        map.set(sku, entry);
      ...
      if (dateFrom && dateTo) {
        ...
        let serverMap = await tmcFetchSummaryMapFromRpc(state, skusLimited);
        if (serverMap && serverMap.size > 0) {
          skus.forEach(sku => resultMap.set(sku, serverMap.get(sku) || { summary: '', qty: null, priorityScore: 0, overdueDates: [], upcomingDates: [], nOrdersSum: 0, nForecasts: 0 }));
      ...
      const worker = async () => {
        while (idx < fallbackSkus.length) {
          ...
          try {
            ...
            if (value) {
              const el = document.querySelector(`.tmc-when-cell[data-sku="${sku}"]`);
    ...
    async function renderProductOverviewTable(ui, rows) {
      ...
      // Show only rows that have BOTH "מתי צפוי" AND "כמה להזמין"
      rows = rows.filter(r => {
        const sku = tmcNormalizeDigits(r?.sku);
        const res = summaryMap?.get(sku) || summaryMap?.get(tmcNormalizeDigits(r?.barcode)) || {};
        const summary = res?.summary || '';
        const qty = res?.qty ?? null;
        return !!summary && summary !== '—' && qty != null;
      });
      loadCatalogCacheOnce();
      ...
      // מוצרים עם תחזית פעילה (יש שורות drilldown) – ראשונים; מוצרים בלי תחזית (למשל "2 הזמנות") – לסוף
      if (rows.length > 0 && rows.length <= 50 && typeof supaRestFetch === 'function') {
        const idsToCheck = new Set();
        rows.forEach(r => {
          const cat = getCatalogForSku(r?.sku || r?.barcode);
          const csku = cat ? String(cat.sku || r?.sku || '').trim() : tmcNormalizeDigits(r?.sku) || '';
          const cbar = cat?.barcode ? tmcNormalizeDigits(cat.barcode) : (r?.barcode ? tmcNormalizeDigits(r.barcode) : '');
          if (csku) idsToCheck.add(csku);
          if (cbar) idsToCheck.add(cbar);
        });
        const idList = Array.from(idsToCheck);
        try {
          const rpcRows = await supaRestFetch('/rest/v1/rpc/get_customer_forecast_for_drilldown', {
            method: 'POST',
            body: { p_skus: idList.length ? idList : null }
          });
          const validIds = new Set(Array.isArray(rpcRows) ? rpcRows.map(dr => tmcNormalizeDigits(dr?.sku)).filter(Boolean) : []);
          rows.forEach(r => {
            const cat = getCatalogForSku(r?.sku || r?.barcode);
            const csku = cat ? String(cat.sku || r?.sku || '').trim() : tmcNormalizeDigits(r?.sku) || '';
            const cbar = cat?.barcode ? tmcNormalizeDigits(cat.barcode) : (r?.barcode ? tmcNormalizeDigits(r.barcode) : '');
            r._hasDrilldown = validIds.has(csku) || (cbar && validIds.has(cbar));
          });
        } catch (_) {
          rows.forEach(r => { r._hasDrilldown = true; });
        }
      } else {
        rows.forEach(r => { r._hasDrilldown = true; });
      }
    ...
    function tmcEnsureDrilldownCssOnce() {
      ...
      style.textContent = `
        ...
        #tmc-drilldown-modal .tmc-panel { position: relative; width: min(980px, calc(100vw - 24px)); max-height: calc(100vh - 24px); overflow: auto; background: #fff; border-radius: 12px; box-shadow: 0 12px 40px rgba(0,0,0,0.25); padding: 0; display: flex; flex-direction: column; }
        ...
        /* טבלה */
        #tmc-drilldown-modal .tmc-body { flex: 1; overflow: auto; min-height: 0; padding: 0; }
        #tmc-drilldown-modal table { width: 100%; border-collapse: collapse; font-size: 13px; }
        #tmc-drilldown-modal th, #tmc-drilldown-modal td { padding: 12px 20px; border-bottom: 1px solid #eaecf0; vertical-align: middle; text-align: right; white-space: nowrap; }
    ...
    async function tmcFetchCustomerForecastBySku(sku, opts = {}) {
      ...
      // ניסיון ל־RPC עם סינון "2 הזמנות ברצף בלי המוצר"
      try {
        const rpcRows = await supaRestFetch('/rest/v1/rpc/get_customer_forecast_for_drilldown', {
          method: 'POST',
          body: { p_skus: candidates }
        });
        if (Array.isArray(rpcRows) && rpcRows.length >= 0) return rpcRows;
      } catch (_) {
        /* RPC לא קיים או שגיאה – fallback ל־view ישיר */
      }

      // Fallback: fetch מ־view ישיר (בלי סינון 2־הזמנות)
      let path = `/rest/v1/v_customer_product_forecast?select=customer_key,customer_name,status,n_orders,last_quantity,last_order_date,avg_gap_days_recent,next_expected_date,days_until_expected` +
                 `&order=customer_name.asc`;
      if (candidates.length > 1) {
        const filters = candidates.map(c => `sku.eq.${encodeURIComponent(c)}`);
        path += `&or=(${filters.join(',')})`;
      } else {
        path += `&sku=eq.${encodeURIComponent(candidates[0] || normalizedSku)}`;
      }
    ...
    unsafeWindow.openProductDrilldown = async function(sku, opts = {}) {
      ...
      // רק לקוחות רלוונטיים לתקופה + בלי היסטוריה כברירת מחדל
      ...
      // אותו סינון כמו בטבלה הראשית: רק שורות עם סטטוס פעיל (מקור יחיד v_customer_product_forecast)
      // חייב להתאים ל־RPC: הטבלה מציגה גם overdue (7 ימים אחורה) – ה־drilldown חייב להציג אותם
      const overdueStart = new Date(today);
      overdueStart.setDate(overdueStart.getDate() - 7);
      overdueStart.setHours(0, 0, 0, 0);
      rows = (rows || []).map(r => ({ ...r, status: normalizeStatus(r) })).filter(r => {
        const status = String(r.status || '').trim();
        if (status !== 'due_or_late' && status !== 'soon' && status !== 'not_yet') {
          return false;
        }
        if (!r.next_expected_date) return true;
        ...
        // או בחלון איחור (7 ימים אחורה) – כמו ב־RPC forecast_product_summary_by_range
        if (d >= overdueStart && d < today) return true;
        return false;
      });
          ...
          <table>
            <thead>
              <tr>
                ...
                <th class="sortable" data-col="last_quantity" title="כמות מההזמנה האחרונה של הלקוח למוצר הזה">${iconFor('last_quantity')}כמות צפויה</th>
                <th class="sortable" data-col="avg_gap_days_recent" title="ממוצע ימים בין ההזמנות האחרונות">${iconFor('avg_gap_days_recent')}מחזור (ימים)</th>
                ...
                <th class="sortable" data-col="next_expected_date" title="התאריך הצפוי הבא לפי המחזור">${iconFor('next_expected_date')}צפי</th>
                <th class="sortable" data-col="days_until_expected" title="כמה ימים נשארו עד הצפי (שלילי = באיחור)">${iconFor('days_until_expected')}ימים</th>
        ...
        for (const r of sorted) {
          ...
          html.push(`
            <tr class="${ui.row}">
              ...
              <td style="color:#475467;">${avgGapDisplay}</td>
              <td dir="ltr" style="color:#475467;">${lastOrder}</td>
              <td dir="ltr" style="font-weight:600; color:#101828;">${nextExpHtml}</td>
              <td style="color:#475467;">${r.days_until_expected ?? '—'}</td>
              <td style="color:#475467;">${r.n_orders ?? 0}</td>
            </tr>
          `);
        }
