    ...
    async function openForecastUI(opts = {}) {
      ...
      async function fetchDataAndRender(stateArg) {
        ...
        try {
          ...
          // =====================================================================
          // OPTION A (Consumption/DOS): main table is built from DOS shortage rows
          // Source of truth: v_sku_dos_shortage_rows_v1 (per customer+sku shortage)
          // We aggregate client-side per SKU to feed normalizeProductRow().
          // =====================================================================
          try {
            const DOS_ROWS_VIEW = 'v_sku_dos_shortage_rows_v1';
            let url = `/rest/v1/${DOS_ROWS_VIEW}` +
              `?select=sku,customer_phone,last_qty,dos_bucket,dos_days,due_date` +
              `&limit=5000`;

            // Filter by selected range using due_date (this is "when stock runs out")
            if (rangeFrom) url += `&due_date=gte.${encodeURIComponent(rangeFrom)}`;
            ...
            for (const r of flat) {
              ...
              const phone9 = tmcPhone9(phone);
              const due = (r?.due_date != null ? String(r.due_date).slice(0, 10) : null);
              const dosDays = (r?.dos_days != null ? Number(r.dos_days) : null);
              const lastQty = (r?.last_qty != null ? Number(r.last_qty) : 0);

              let agg = bySku.get(sku);
              if (!agg) {
                agg = {
                  sku,
                  product_name: sku, // will be enriched later by catalog
                  supplier: null, // will be enriched later by catalog
                  suggested_cycle_days: null,
                  qty_within_cycle: 0,
                  n_records: 0,
                  first_due_date: due,
                  last_due_date: due,
              ...
              // unique customers (phone9)
              if (phone9 && !agg._phone9set.has(phone9)) {
                agg._phone9set.add(phone9);
                agg.customer_keys.push(phone);
                agg.n_records += 1;
              }

              // qty sum (rough "impact" proxy)
              if (Number.isFinite(lastQty) && lastQty > 0) agg.qty_within_cycle += lastQty;

              // min DOS
              if (Number.isFinite(dosDays)) {
                if (agg._minDos == null || dosDays < agg._minDos) agg._minDos = dosDays;
              }

              // nearest due date
              if (due) {
                if (!agg._nearestDue || due < agg._nearestDue) agg._nearestDue = due;
              }
            }

            data = Array.from(bySku.values()).map(x => {
              // normalizeProductRow expects these date fields (ISO YYYY-MM-DD)
              const nearest = x._nearestDue || x.first_due_date || x.last_due_date || null;
              x.first_due_date = nearest;
              x.last_due_date = nearest;

              // Never leak Set into normalized rows (can break later JSON ops)
              try { delete x._phone9set; } catch (_) {}
              return x;
            });

          } catch (e) {
            console.warn('[Forecast] DOS rows view fetch failed; falling back to legacy sources', e);
            data = null;
          }
