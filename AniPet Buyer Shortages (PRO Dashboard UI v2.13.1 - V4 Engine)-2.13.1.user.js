// ==UserScript==
// @name         AniPet Buyer Shortages (PRO Dashboard UI v2.13.1 - V4 Engine)
// @namespace    anipet.buyer
// @version      2.13.1
// @description  Fixed tooltip offset math. Fully synced manual refresh trigger.
...
(() => {
  ...
  const css = `
    ...
    /* ======== CSS FIX FOR ACTION ICONS (#007bff) ======== */
    .ani-action-btn {
        ...
        cursor: pointer !important; padding: 0 2px !important;
    }
    .ani-action-btn i {
        color: #007bff !important; font-size: 15px !important;
        line-height: 1 !important;
    }
    .ani-action-btn:hover, .ani-action-btn:hover i {
        color: #0056b3 !important; }
    .ani-actions-container a.ani-action-btn i.fa-brands,
    .ani-actions-container a.ani-action-btn i.fa-light,
    .ani-actions-container a.ani-action-btn i.fa-solid {
        color: #007bff !important; font-size: 15px !important;
        line-height: 1 !important;
    }
    .ani-actions-container a.ani-action-btn:hover i {
        color: #0056b3 !important; }

    /* ======== CSS FIX FOR IMAGE HOVER & CLICK ======== */
    .ani-thumb-img {
  ...
  // ======== IMAGE LOGIC (CLEAN & COPY TO CLIPBOARD) ======== //
  ...
  async function copyImageToClipboard(imgElement, rawUrl) {
      // Use original raw URL if available, otherwise parse the src
      ...
      try {
          // Pass through wsrv.nl to force PNG format and bypass CORS constraints for Clipboard API
          ...
          console.error(`[${SCRIPT_NAME}] Failed to copy image to clipboard. Opening in new tab instead.`, err);
  ...
  const headerDiv = el("div", { id: "anipet-pro-header" }, [
    el("div", { style: { display: "flex", alignItems: "center", gap: "12px" } }, [
      ...
      el("div", {}, [
        ...
        el("div", { textContent: "V2.13.1 - V4 Smart Engine (Synced)", style: { fontSize: "12px", color: "#94a3b8" } })
      ])
    ]),
  ...
  function applyPreset(p) {
      ...
      state.graceDays = p.grace; state.windowDays = p.window;
      state.sortBy = p.sort;
  ...
  toolbar.appendChild(controlsRow1); toolbar.appendChild(controlsRow2);
  ...
  function buildSkuCard(s, isSearchMode = false) {
      ...
      const thumbnailEl = el("img", {
          ...
          className: "ani-thumb-img",
          title: "לחץ להעתקת התמונה בגרסה המלאה ללוח",
          style: { width: "50px", height: "50px", objectFit: "contain", borderRadius: "6px", border: "1px solid #e2e8f0", background: "#fff", cursor: "pointer" },
          onerror: function() { if(!this.dataset.fallbackTriggered && rawImgUrl) { this.dataset.fallbackTriggered = 'true'; this.src = rawImgUrl; } }
      });
      thumbnailEl.dataset.originalSrc = rawImgUrl;
      thumbnailEl.onclick = (e) => {
          e.stopPropagation();
          copyImageToClipboard(thumbnailEl, rawImgUrl);
      };
  ...
  async function renderCustomersForSku(users, containerElement) {
      try {
          ...
          for (const u of users) {
              ...
              // התיקון לבאג ה-0 ימים שהורדנו את ה-Math.max
              let offsetDays = Math.round(daysTotal - u.dos_days);
              ...
              let explainText = `<div style="font-size:14px; font-weight:bold; margin-bottom:8px; border-bottom:1px solid #475569; padding-bottom:4px;">מנוע V4 - איך חושב?</div>`;
