        ...
        if (didWork) {
          try {
            ...
            if (matchedTaskIds.length > 0) {
              // שולחים בבאצ'ים קטנים כדי להפחית כשלים בפונקציית RPC על payload גדול
              const RPC_BATCH_SIZE = 200;
              for (let i = 0; i < matchedTaskIds.length; i += RPC_BATCH_SIZE) {
                const chunkIds = matchedTaskIds.slice(i, i + RPC_BATCH_SIZE);
                await supaRestFetch('/rest/v1/rpc/match_forecast_predictions_batch', {
                  method: 'POST',
                  body: { p_task_ids: chunkIds },
                });
              }
            } else {
