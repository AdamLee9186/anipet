...
function __tmcSetRowBlink(tr, shouldBlink){
  ...
  // Single source of truth for "missing/invalid phone" state:
  // - tmc-phone-blink: animation (CSS)
  // - data-phone-blink: cheap state check (no computedStyle / reflow)
  // - phone-missing-row-highlight: optional static highlight class
  if (shouldBlink){
    tr.classList.add('tmc-phone-blink');
    tr.classList.add('phone-missing-row-highlight');
    try { tr.dataset.phoneBlink = '1'; } catch(_){}
  } else {
    tr.classList.remove('tmc-phone-blink');
    tr.classList.remove('phone-missing-row-highlight');
    try { if (tr.dataset) delete tr.dataset.phoneBlink; } catch(_){}
  }
}
...
function validatePhonesEverywhere(root = document){
  try{
    ...
    // NOTE: This function must be cheap because it can be triggered often.
    // We avoid:
    // - innerText (can force layout)
    // - full re-processing when the phone text hasn't changed
        scope.querySelectorAll('table').forEach(tmcEnsureDataLabelsForTable);
...
const cells = scope.querySelectorAll('td[data-label="טלפון"]');
    ...
    cells.forEach(td => {
      ...
      if (!isIntl) {
        // Blink rules:
        // 1) length must be 9 or 10
        if (digits.length < 9 || digits.length > 10) {
          shouldBlink = true;
        } else {
          // 2) must start with 0, except a forgivable 9-digit mobile missing the leading 0 ("5xxxxxxxx")
...
(function __tmcBootstrapTableFixes(){
  ...
  const run = () => {
    try{
      ...
      validatePhonesEverywhere(document);
...
(function observeTablesEverywhere(){
  ...
  const mo = new MutationObserver(muts => {
    ...
    for (const m of muts) {
      // Ignore most attribute changes to avoid loops (class, style changes),
      // but allow phone-related attributes to trigger re-validation.
      if (m.type === 'attributes') {
        const attr = m.attributeName || '';
        if (attr === 'title' || attr === 'data-original-title' || attr === 'href') {
          const el = m.target;
          const td = el && el.closest ? el.closest('td[data-label="טלפון"]') : null;
          if (td) {
            touched = true;
            if (td.closest('.offcanvas, .offcanvas-right, .offcanvas-custom, #panel_view')) {
              touchedPanel = true;
              touchedPhonesPanel = true;
            }
            if (td.closest('#operator-store-visits-table')) {
              touchedTable = true;
            }
          }
        }
    ...
    if (touched) {
      ...
      if (panel && (touchedPanel || touchedPhonesPanel)) {
        schedulePanel(panel);
        if (touchedPhonesPanel) validatePhonesEverywhere(panel);
      }
    }
  });
...
// Removed: this block used getComputedStyle()/deep DOM scans on an interval, causing forced reflow + ticking.
// Row state is now handled directly in __tmcSetRowBlink() + validatePhonesEverywhere().
