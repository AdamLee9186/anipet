// ==UserScript==
// @name         AniPet Buyer Shortages (PRO Dashboard UI v2.8.0 - In-Memory Aggregation)
// @namespace    anipet.buyer
// @version      2.8.0
// @description  Perfect Macro-Micro sync, 100-day max gap filter, unified reliability labels, negative grace days.
...
(() => {
  ...
  const css = `
    ...
    .ani-tooltip-box { position: fixed; background: rgba(15, 23, 42, 0.95); color: #fff; padding: 12px; border-radius: 8px; font-size: 13px; font-weight: 500; pointer-events: none; z-index: 999999999; opacity: 0; transition: opacity 0.2s; box-shadow: 0 10px 25px rgba(0,0,0,0.2); max-width: 300px; text-align: right; direction: rtl; line-height: 1.6; border: 1px solid #334155; }
  ...
  const PRESETS_HARDCODED = [
    { id: 'buyer_action', label: '🛒 רכש SOS (מוצרים קריטיים)', grace: 0, window: 14, sort: 'smart', filterStatus: 'all', filterAnimal: 'all', filterType: 'all', filterRel: 'all', filterSupplier: 'all', tooltip: "מציג רק את הלחם והחמאה של החנות, ללא רעשי רקע." },
    { id: 'upcoming', label: '🔭 תחזית עתידית (חודש קדימה)', grace: 0, window: 30, sort: 'upcoming', filterStatus: 'all', filterAnimal: 'all', filterType: 'all', filterRel: 'all', filterSupplier: 'all', tooltip: "מבט רחוק: כל המוצרים שיידרשו בחודש הקרוב." },
  ...
  const headerDiv = el("div", { id: "anipet-pro-header" }, [
    el("div", { style: { display: "flex", alignItems: "center", gap: "12px" } }, [
      ...
      el("div", {}, [
        ...
        el("div", { textContent: "V2.8.0 - סנכרון מאקרו מושלם וסינון תדירות קשוח", style: { fontSize: "12px", color: "#94a3b8" } })
      ])
    ]),
  ...
  const inGrace = el("input", { type: "number", className: "ani-input", style: { width: "70px" }, value: state.graceDays }); // Removed min="0" to allow negative
  ...
  const selRel = el("select", { className: "ani-select", style: { width: "160px" } });
  [{v:'all', l:'🎯 כל האמינויות'}, {v:'swiss', l:'⏱️ שעון שוויצרי'}, {v:'regular', l:'✓ סדיר'}, {v:'erratic', l:'🎲 כאוטי'}, {v:'sparse', l:'❓ מעט נתונים'}].forEach(o => selRel.appendChild(el("option", { value: o.v, textContent: o.l, selected: state.filterCustomerRel === o.v })));
  ...
  // Instant UI Updates for all drop downs! No network calls needed anymore.
  ...
  // ================================================
  // NEW ARCHITECTURE: IN-MEMORY AGGREGATION
  // ================================================
  ...
  function renderMainList() {
    ...
    // 1. FILTER IN MEMORY FIRST (This fixes the Macro/Micro bug)
    let filteredRows = state.allRawData.filter(r => {
        ...
        // 100-Day Rule: Filter out "Ghosts" who buy less frequently than 100 days
        if (r.avg_gap_days > 100) return false;

        // Apply UI Filters
        if (state.filterAnimal !== 'all' && info.animal !== state.filterAnimal) return false;
        if (state.filterType !== 'all' && info.type !== state.filterType) return false;
        if (state.filterSupplier !== 'all' && sSup !== state.filterSupplier) return false;
        if (state.filterCustomerStatus !== 'all' && r.dos_bucket !== state.filterCustomerStatus) return false;
        if (state.filterCustomerRel !== 'all' && relStatus !== state.filterCustomerRel) return false;
    ...
    // 2. AGGREGATE INTO SKUS (Dynamic Macro Cards)
    const skuMap = {};
    ...
    // 3. APPLY SOS FILTER (Junk Removal)
    aggregatedSkus = aggregatedSkus.filter(s => {
        let isImportant = true;
        if (state.activePreset === 'buyer_action' || state.sortBy === 'smart') {
  ...
  // Instant render of customers already grouped in JS memory
  async function renderCustomersForSku(users, containerElement) {
      try {
          const phones = users.map(u => u.customer_phone);
          ...
          for (const u of users) {
              ...
              let dateNode = el("div", { className: "has-tooltip", "data-tooltip": explainText, style: { display: "flex", alignItems: "center", gap: "6px", color: "#0f172a", fontWeight: "600" } }, [
                  el("span", { textContent: fmtDate(u.next_expected_date) }),
                  el("i", { className: "fa-solid fa-calculator", style: { color: "#3b82f6", fontSize: "12px", cursor: "help" } })
              ]);
              ...
              custRows.push([
                displayPhone, cName, u.customer_addr || "",
  ...
  function renderCustomerList() {
      ...
      let filtered = allCustomers.filter(c => {
          ...
          for(const cat of c.categories) {
              ...
              if (cat.avg_gap_days > 100) continue; // 100 Day Ghost Rule
              ...
              const mRel = state.filterCustomerRel === 'all' || relStatus === state.filterCustomerRel;
  ...
  function buildCustomerCard(phone, name, addr, categories, autoOpen = false) {
      ...
      btnToggle.onclick = async () => {
        if (cBody.classList.contains('open')) { cBody.classList.remove('open'); btnToggle.textContent = "פתח פירוט"; }
            ...
            if (!isLoaded) { cBody.innerHTML = ""; cBody.appendChild(createSkeletonTable()); await renderCustomersForSku(categories, cBody); isLoaded = true; } // Reuse same renderer!
        }
      };
