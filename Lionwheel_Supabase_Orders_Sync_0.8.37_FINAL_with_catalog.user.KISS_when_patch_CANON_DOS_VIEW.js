...
(function() {
    ...
    // Public debug bridge: expose markers to page context and DOM (userscript-safe).
    const __tmcPublicWindow = (typeof unsafeWindow !== 'undefined' && unsafeWindow) ? unsafeWindow : window;
    try {
      const boot = { loadedAt: Date.now(), version: '0.8.33' };
      __tmcPublicWindow.__tmcScriptLoaded = boot;
      document.documentElement.setAttribute('data-tmc-script-loaded', JSON.stringify(boot));
    } catch (_) {}
    ...
    const ORDERS_SYNC_CONFIG = {
      ...
      // כדי לצמצם HTTP 429, נריץ משימות בצורה טורית (ללא burst).
...
// --- קונפיגורציה לקאש (בהשראת Toolbox.js) ---
  ...
  function getProductThumbUrl(skuLike) {
    ...
    function extractStoreVisitsFromIndexTable() {
      ...
      return rows.map((tr) => {
        ...
        let status_text = '';
        let pick_status_text = '';
        if (tdStatus) {
          const mainStatusSpan =
          ...
          const pickStatusSpanInStatusCell =
            tdStatus.querySelector('button.pick-status .pick-status-text')
            || tdStatus.querySelector('.pick-status-text');

          if (mainStatusSpan) {
            status_text = textFrom(mainStatusSpan).trim();
          } else {
            // Fallback only when specific status selector is missing.
            status_text = lwFirstLine(textFrom(tdStatus)) || '';
          }

          if (pickStatusSpanInStatusCell) {
            pick_status_text = textFrom(pickStatusSpanInStatusCell).trim();
          }
        }
        if (!pick_status_text && tdPick) {
          const pickStatusSpan =
            tdPick.querySelector('button.pick-status .pick-status-text')
            || tdPick.querySelector('.pick-status-text');
          pick_status_text = pickStatusSpan
            ? textFrom(pickStatusSpan).trim()
            : (lwFirstLine(textFrom(tdPick)) || '');
        }
  ...
  async function buildStoreVisitsRawRecords(silent = false) {
    ...
    async function syncStoreVisitsIndexToSupabase({
      ...
      const syncedTaskIds = [];
      const successfulSyncedTaskIds = [];
      let taskProgressTotal = 0;
      let taskProgressDone = 0;
      let taskProgressFail = 0;
      let taskProgressSuccess = 0;
      ...
      try {
        ...
        if (alsoSyncTasks && taskIds.length) {
          taskProgressTotal = taskIds.length;
          taskProgressDone = 0;
          taskProgressFail = 0;
          taskProgressSuccess = 0;
          lwUpdateTaskProgressToast(0, taskProgressTotal, 0);
          ...
          const statusByTaskId = new Map(
            rows.map((r) => [String(r.task_id || '').trim(), lwFirstLine(r.status_text)])
          );
          ...
          for (let i = 0; i < taskIds.length; i += TASK_BATCH_SIZE) {
            ...
            taskProgressDone += chunk.length;
            taskProgressFail += chunkResults.reduce((acc, r) => acc + (r.status === 'rejected' ? 1 : 0), 0);
            lwUpdateTaskProgressToast(taskProgressDone, taskProgressTotal, taskProgressFail);
          ...
          results.forEach((r, i) => {
            ...
            if (r.status === 'fulfilled') {
              ...
              taskProgressSuccess++;
              successfulSyncedTaskIds.push(String(tid));
          ...
          lwFinishTaskProgressToast(taskProgressSuccess, taskProgressTotal, fail);
        ...
        lwCloseTaskProgressToast(true);
      } finally {
        // match_forecast_predictions: שידוך תחזיות עם הזמנות (לעדכון fulfilled/missed)
        if (didWork) {
          try {
            const matchedTaskIds = Array.from(
              new Set(
                (successfulSyncedTaskIds.length ? successfulSyncedTaskIds : syncedTaskIds)
                  .map((v) => String(v || '').trim())
                  .filter(Boolean)
              )
            );
            ...
            if (matchedTaskIds.length > 0) {
              const matchStats = await runMatchForecastPredictionsBatchAdaptive(matchedTaskIds, {
                initialChunkSize: 200,
                minChunkSize: 20,
              });
              if (matchStats.timedOut && LW_ORDERS_CONFIG.DEBUG) {
                console.warn(
                  '[Supabase Sync] match_forecast_predictions_batch completed with adaptive splits:',
                  matchStats
                );
              }
            } else {
        ...
        if (!alsoSyncTasks || taskProgressTotal <= 0) {
          lwCloseTaskProgressToast(true);
        }
      }
    }
    ...
    async function fetchTaskDocument(taskId) {
      ...
      // Throttle task-page fetches to reduce Lionwheel rate-limit (HTTP 429).
      await lwWait(800);
    ...
    function ensureToastStyles() {
      ...
      style.textContent = `
        ...
        .lw-toast.lw-toast--progress { min-width: 300px; max-width: 440px; pointer-events: auto; }
        .lw-toast-progress-title { font-weight: 700; margin-bottom: 6px; }
        .lw-toast-progress-meta { font-size: 12px; opacity: 0.95; margin-bottom: 6px; }
        .lw-toast-progress-track {
          width: 100%;
          height: 8px;
          border-radius: 999px;
    ...
    function lwCloseTaskProgressToast(immediate = false) {
      if (__lwTaskProgressRemoveTimer) {
        clearTimeout(__lwTaskProgressRemoveTimer);
        __lwTaskProgressRemoveTimer = null;
      }
      if (!__lwTaskProgressToastEl) return;
      const el = __lwTaskProgressToastEl;
      __lwTaskProgressToastEl = null;
      if (immediate) {
        el.remove();
        return;
      }
    ...
    function lwEnsureTaskProgressToast(total) {
      ensureToastStyles();
      if (__lwTaskProgressRemoveTimer) {
        clearTimeout(__lwTaskProgressRemoveTimer);
        __lwTaskProgressRemoveTimer = null;
      }
      if (__lwTaskProgressToastEl && __lwTaskProgressToastEl.isConnected) return __lwTaskProgressToastEl;

      const safeTotal = Math.max(0, Number(total) || 0);
      const el = document.createElement('div');
      el.className = 'lw-toast lw-toast--info lw-toast--progress';
      el.innerHTML = `
        <div class="lw-toast-progress-title">סנכרון משימות ל-Supabase</div>
    ...
    function lwUpdateTaskProgressToast(done, total, fail = 0) {
      const safeTotal = Math.max(0, Number(total) || 0);
      if (safeTotal <= 0) {
        lwCloseTaskProgressToast(true);
        return;
      ...
      const el = lwEnsureTaskProgressToast(safeTotal);
      const meta = el.querySelector('.lw-toast-progress-meta');
      const fill = el.querySelector('.lw-toast-progress-fill');
      const pct = Math.round((safeDone / safeTotal) * 100);
      if (meta) {
        meta.textContent = fail > 0
          ? `${safeDone}/${safeTotal} (${pct}%) • כשלים: ${fail}`
          : `${safeDone}/${safeTotal} (${pct}%)`;
      }
      if (fill) fill.style.width = `${pct}%`;
    }

    function lwFinishTaskProgressToast(successfulCount, total, fail = 0) {
      const safeTotal = Math.max(0, Number(total) || 0);
      if (safeTotal <= 0) {
        lwCloseTaskProgressToast(true);
        return;
      }
      const safeSuccess = Math.max(0, Number(successfulCount) || 0);
      const el = lwEnsureTaskProgressToast(safeTotal);
      const title = el.querySelector('.lw-toast-progress-title');
      const meta = el.querySelector('.lw-toast-progress-meta');
      const fill = el.querySelector('.lw-toast-progress-fill');
      const ok = fail <= 0;

      el.classList.remove('lw-toast--info');
      el.classList.add(ok ? 'lw-toast--success' : 'lw-toast--warning');
      if (title) title.textContent = ok ? 'סנכרון משימות הושלם' : 'סנכרון משימות הושלם עם כשלים';
      if (meta) meta.textContent = `${safeSuccess}/${safeTotal} הצליחו${fail > 0 ? ` • ${fail} נכשלו` : ''}`;
      if (fill) fill.style.width = '100%';

      __lwTaskProgressRemoveTimer = window.setTimeout(() => {
        lwCloseTaskProgressToast(false);
      }, 1800);
    }
    ...
    function showToast(text, type = 'info', ms = 2800) {
      ...
      const looksLikeProblem = /(שגיאה|נכשל|error|failed|exception|timeout)/i.test(rawText);
      const isErrorType = type === 'error';
      const isWarnType = type === 'warning' || type === 'warn';
      const baseMs = Number.isFinite(Number(ms)) ? Number(ms) : 2800;
      const timeoutMs = isErrorType || looksLikeProblem
        ? Math.max(baseMs, 12000)
        : isWarnType
    ...
    function isSupabaseStatementTimeoutError(err) {
      const msg = String(err?.message || err || '');
      return msg.includes('"code":"57014"') || /statement timeout/i.test(msg);
    }

    async function runMatchForecastPredictionsBatchAdaptive(taskIds, {
      initialChunkSize = 200,
      minChunkSize = 20,
    } = {}) {
      ...
      if (!ids.length) return { ok: 0, failed: 0, timedOut: false };

      const queue = [];
      for (let i = 0; i < ids.length; i += initialChunkSize) {
        queue.push(ids.slice(i, i + initialChunkSize));
      }
      ...
      while (queue.length) {
        ...
        try {
          ...
          if (isSupabaseStatementTimeoutError(e)) {
            ...
            if (LW_ORDERS_CONFIG.DEBUG) {
              console.warn(
                '[Supabase Sync] match_forecast_predictions_batch timeout on minimal chunk; skipped chunk size=',
                chunk.length
              );
            }
            continue;
    ...
    function normalizeProductRow(r, i) {
      ...
      return {
        ...
        // DOS / Smart qty fields:
        // Preserve precomputed Option-A values if present, otherwise default to 0.
        _critCount: Number(r?._critCount) || 0,
        _lowCount: Number(r?._lowCount) || 0,
        _qtyCrit: Number(r?._qtyCrit) || 0,
        _qtyLow: Number(r?._qtyLow) || 0,
        _categoryKey: null,
      };
    }
    ...
    function getFilteredAndSortedProductRows(state) {
      ...
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      const usedDosOptionA = (state?._usedDosOptionA === true);
      ...
      rows = rows.filter(row => {
        ...
        if (range) {
          ...
          if (usedDosOptionA) {
            // Option A already fetched DOS shortages with a backward due_date buffer.
            // Keep small overdue window visible in UI instead of clipping by strict range.from.
          ...
          if (start && end) {
            if (end < rangeFrom || start > rangeTo) return false;
          } else if (start) {
            if (start < rangeFrom || start > rangeTo) return false;
          } else if (end) {
            if (end < rangeFrom || end > rangeTo) return false;
          }
        }
      ...
      if (onlyShortage && signalsReady) {
        if (usedDosOptionA) {
          // Option A already comes from DOS shortage rows (LOW/CRIT + open).
          // If client reconciliation temporarily zeroes crit/low, keep rows with positive qty.
          rows = rows.filter(r => {
            const crit = Number(r._critCount) || 0;
            const low = Number(r._lowCount) || 0;
            const qty = Number(r.qty_within_cycle) || 0;
            return (crit > 0 || low > 0 || qty > 0);
          });
        } else {
    ...
    async function openForecastUI(opts = {}) {
      ...
      async function fetchDataAndRender(stateArg) {
        // -------------------------------------------------------------------
        // Debug anchor: set both on window AND on DOM so it's visible from
        // page console even when script runs in userscript isolated world.
        // In console run: JSON.parse(document.documentElement.getAttribute('data-tmc-forecast-debug') || '{}')
        // -------------------------------------------------------------------
        const publicWin = (typeof unsafeWindow !== 'undefined' && unsafeWindow) ? unsafeWindow : window;
        function setTmcForecastDebug(extra) {
          try {
            const el = document.documentElement;
            const prev = (el.getAttribute && el.getAttribute('data-tmc-forecast-debug')) || '{}';
            const cur = Object.assign({}, JSON.parse(prev), extra);
            el.setAttribute('data-tmc-forecast-debug', JSON.stringify(cur));
            publicWin.__tmcForecastDebug = cur;
          } catch (_) {}
        }
        try {
          publicWin.__tmcForecastState = stateArg;
          publicWin.__tmcForecastDebug = publicWin.__tmcForecastDebug || {};
          publicWin.__tmcForecastDebug.startedAt = Date.now();
          publicWin.__tmcForecastDebug.step = 'start';
          setTmcForecastDebug({ step: 'start', startedAt: Date.now() });
        } catch (_) {}
        ...
        try {
          ...
          let data = null;
          let usedDosOptionA = false;

          // =====================================================================
          // OPTION A (Consumption/DOS): main table is built from DOS detail rows
          // Source of truth: v_dos_details_per_customer_sku_v3 (works over REST)
          // We aggregate per SKU and MUST provide qty_within_cycle & cycle_days
          // so getFilteredAndSortedProductRows() won't filter everything out.
          // =====================================================================
          try {
            try { publicWin.__tmcForecastDebug.step = 'optionA_fetch_begin'; setTmcForecastDebug({ step: 'optionA_fetch_begin' }); } catch (_) {}
            const DOS_ROWS_VIEW = 'v_dos_details_per_customer_sku_v3';

            // Pull only shortage-like rows (LOW/CRIT) and only OPEN (not fulfilled/missed)
            let url = `/rest/v1/${DOS_ROWS_VIEW}` +
              `?select=sku,customer_phone,last_qty,dos_bucket,dos_days,due_date,outcome_status` +
              `&dos_bucket=in.(LOW,CRIT)` +
              `&outcome_status=eq.open` +
              `&limit=5000`;

            // IMPORTANT: range filter should usually be "up to rangeTo".
            // If you filter by gte(rangeFrom) you can lose items already due (like 2026-02-16).
            // We'll keep an optional small back-buffer for rangeFrom.
            if (rangeTo) url += `&due_date=lte.${encodeURIComponent(rangeTo)}`;
            if (rangeFrom) {
              try {
                const d = new Date(rangeFrom);
                d.setDate(d.getDate() - 14); // 14d buffer backwards
                const backFrom = d.toISOString().slice(0, 10);
                url += `&due_date=gte.${encodeURIComponent(backFrom)}`;
              } catch (_) {
                // ignore
              }
            }

            const rows = await supaRestFetchPaged(url, { method: 'GET' });
            const flat = Array.isArray(rows) ? rows : [];
            try {
              publicWin.__tmcForecastDebug.optionA = { url, rowsN: flat.length };
              publicWin.__tmcForecastDebug.step = 'optionA_fetch_ok';
              setTmcForecastDebug({ step: 'optionA_fetch_ok', optionA: { url, rowsN: flat.length } });
            } catch (_) {}
            ...
            for (const r of flat) {
              ...
              if (!g) {
                g = {
                  sku,
                  product_name: sku,          // will be enriched later (optional)
                  supplier: null,
                  suggested_cycle_days: null, // we will set below
                  qty_within_cycle: 0,        // MUST be >0 to survive filters
                  n_records: 0,
                  first_due_date: null,
                  last_due_date: null,
                  n_missed: 0,
                ...
                g._phone9set = new Set();
                g._minDos = null;
                g._nearestDue = null;
                bySku.set(sku, g);
              }

              // qty_within_cycle: sum last_qty for shortage customers (coarse but works for UI)
              const q = Number(r?.last_qty);
              if (Number.isFinite(q) && q > 0) g.qty_within_cycle += q;

              const bucket = String(r?.dos_bucket || '').toUpperCase();
              if (bucket === 'CRIT') {
                g._critCount += 1;
                if (Number.isFinite(q) && q > 0) g._qtyCrit += q;
              } else if (bucket === 'LOW') {
                g._lowCount += 1;
                if (Number.isFinite(q) && q > 0) g._qtyLow += q;
              }

              // customer_keys: MUST be phone9 (not raw phone) so joins/gatekeeper match
              const phone = String(r?.customer_phone || '').trim();
              const p9 = tmcPhone9(phone);
              if (p9) g._phone9set.add(p9);

              // suggested_cycle_days: use min dos_days as a proxy (so column not empty)
              const dos = Number(r?.dos_days);
              if (Number.isFinite(dos) && dos >= 0) {
                g._minDos = (g._minDos == null) ? dos : Math.min(g._minDos, dos);
              }

              // window dates: keep nearest due date
              const dd = r?.due_date ? String(r.due_date) : null;
              if (dd) {
                if (!g._nearestDue || dd < g._nearestDue) g._nearestDue = dd;
              }
            }

            // Finalize groups into rows the existing UI expects
            data = Array.from(bySku.values()).map(g => {
              const nearest = g._nearestDue || null;
              g.first_due_date = nearest;
              g.last_due_date = nearest;

              // If qty ended up 0 for some reason, set to count of shortage customers as a fallback
              if (!(Number(g.qty_within_cycle) > 0)) g.qty_within_cycle = Math.max(1, g.n_records);

              // Provide a cycle_days value to avoid downstream filters/blank UI
              g.suggested_cycle_days = (g._minDos != null) ? Math.round(g._minDos) : 0;

              // IMPORTANT: customer_keys MUST be phone9 values (not raw phones)
              g.customer_keys = Array.from(g._phone9set || []);
              g.n_records = g.customer_keys.length;

              // Safety: Option A should always look like active shortage rows
              if (((Number(g._critCount) || 0) + (Number(g._lowCount) || 0)) <= 0) {
                g._lowCount = Math.max(1, g.n_records || 1);
              }
              if ((Number(g._qtyCrit) || 0) <= 0 && (Number(g._qtyLow) || 0) <= 0) {
                g._qtyLow = Number(g.qty_within_cycle) || Math.max(1, g.n_records || 1);
              }

              try { delete g._phone9set; delete g._minDos; delete g._nearestDue; } catch (_) {}
              return g;
            });
            usedDosOptionA = true;
            try { setTmcForecastDebug({ optionAGroupedSkus: data.length, usedDosOptionA: true }); } catch (_) {}

          } catch (e) {
            console.warn('[Forecast] DOS details view fetch failed; falling back to legacy sources', e);
            try {
              publicWin.__tmcForecastDebug.step = 'optionA_fetch_failed';
              publicWin.__tmcForecastDebug.optionAError = String(e?.message || e);
              setTmcForecastDebug({ step: 'optionA_fetch_failed', optionAError: String(e?.message || e) });
            } catch (_) {}
            data = null;
          }

          // Legacy fallback (only if DOS source failed)
          ...
          try { setTmcForecastDebug({ normalizedRowsBeforeSignals: normalizedRows.length, usedDosOptionA }); } catch (_) {}
          ...
          if (!usedDosOptionA) {
            try {
              ...
              normalizedRows.forEach(row => {
                ...
                row._evidenceAllFulfilled = false;
                ...
                // IMPORTANT: Do not override signals / n_open if we failed to match any details.
                // Empty match MUST NOT be treated as "0 open" (otherwise we delete valid rows).
                if (!filteredDetails || !filteredDetails.length) return;
                ...
                // Evidence flag: only TRUE when we actually saw details and none are open.
                if (clientOpenCount === 0) row._evidenceAllFulfilled = true;
              ...
              // The Gatekeeper (REMOVAL-ONLY):
              // Remove ONLY when we have positive evidence that ALL matched details are fulfilled.
              // Never remove just because matching failed or yielded empty details.
              normalizedRows = normalizedRows.filter(r => {
                if (r._hasClientDetails && r._evidenceAllFulfilled === true) return false;
                return true;
              });
            ...
            // Option A comes from shortage/open DOS rows, but we still reconcile with outcomes
            // to drop rows that became fulfilled very recently.
            try {
              const beforeReconcileRows = normalizedRows.slice();
              const selectedKey = stateArg?.dateRangeKey || null;
              const dateTo = stateArg?.dateTo || null;
              const isAllOpen = (selectedKey === 'כל הפתוחים');
              let dateFromForCheck = stateArg?.dateFrom || null;
              if (!isAllOpen && dateFromForCheck) {
                try {
                  const d = new Date(dateFromForCheck);
                  d.setDate(d.getDate() - 14);
                  dateFromForCheck = d.toISOString().slice(0, 10);
                } catch (_) {}
              }
              ...
              normalizedRows.forEach(row => {
                ...
                const filteredDetails = (entries.__deduped || entries).filter((e) => {
                  const phone9 = tmcPhone9(e?.customer_phone);
                  if (!phone9 || !keys.has(phone9)) return false;
                  if (!isAllOpen && !tmcIsDateInRangeInclusive(e?.due_date, dateFromForCheck, dateTo)) return false;
                  return true;
                });

                // If reconciliation found no matching details, keep original Option-A signals
                // and avoid zeroing the row by gatekeeper.
                ...
                const clientOpenCount = openRows.length;
                row.n_open = clientOpenCount;
                row._evidenceAllFulfilled = (filteredDetails.length > 0 && clientOpenCount === 0);

                const s = tmcComputeShortageFromDetails({ __deduped: openRows });
                row._critCount = s.critCount;
                row._lowCount = s.lowCount;
                row._qtyCrit = s.qtyCrit;
                row._qtyLow = s.qtyLow;
              ...
              // OPTION A removal-only reconciliation:
              // remove only with explicit evidence that all matched details are fulfilled.
              normalizedRows = normalizedRows.filter(r => {
                if (!r?._hasClientDetails) return true;
                const clientOpen = Number(r.n_open ?? NaN);
                if (!Number.isFinite(clientOpen)) return true;
                if (clientOpen === 0) {
                  if (r._evidenceAllFulfilled === true) return false;
                  return true;
                }
                return true;
              });

              // Safety net: never leave Option A completely empty due to reconciliation mismatch.
              if (!normalizedRows.length && beforeReconcileRows.length) {
                normalizedRows = beforeReconcileRows;
                try { setTmcForecastDebug({ step: 'optionA_reconcile_fallback_keep_original', rowsAfterReconcile: 0, rowsRestored: beforeReconcileRows.length }); } catch (_) {}
              }

              stateArg._signalsReady = true;
              try { setTmcForecastDebug({ step: 'optionA_reconciled_with_outcomes', rowsAfterReconcile: normalizedRows.length }); } catch (_) {}
            } catch (e) {
              console.warn('[Forecast] Option A reconciliation failed; keeping Option A rows as-is', e);
              stateArg._signalsReady = true;
            }
          }
          ...
          stateArg._usedDosOptionA = usedDosOptionA;

          if (typeof publicWin !== 'undefined') publicWin.__tmcForecastState = stateArg;
          try { setTmcForecastDebug({ productOverviewRowsLength: normalizedRows.length, usedDosOptionA: stateArg._usedDosOptionA }); } catch (_) {}
