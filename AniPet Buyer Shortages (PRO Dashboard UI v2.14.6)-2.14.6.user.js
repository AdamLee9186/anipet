// ==UserScript==
// @name         AniPet Buyer Shortages (PRO Dashboard UI v2.14.6)
// @namespace    anipet.buyer
// @version      2.14.6
// @description  Cross-Script RPC Bridge for Instant Notes Highlighting.
...
(() => {
  ...
  const state = {
      viewMode: 'products', limit: 3000, sortBy: 'smart', graceDays: 0, windowDays: 14,
      ...
      feedbackDaysBack: 0, notesIndex: new Set()
  };
      ...
      try {
          ...
          if(res.ok) {
              ...
              if(data && data.length && data[0].last_refresh) {
                  ...
                  syncDiv.innerHTML = `מנוע V4 חושב לאחרונה: <b>${dStr}</b> <div id="ani-refresh-btn" class="sync-refresh-btn" title="רענן מנוע תחזיות עכשיו"><i class="fa-solid fa-arrows-rotate"></i> רענן ידנית</div>`;
                  document.getElementById('ani-refresh-btn').onclick = triggerManualRefresh;
                  return;
              }
          }
          syncDiv.innerHTML = `לא נמצאו נתוני סנכרון למנוע V4`;
      } catch(e){ document.getElementById('ani-sync-time').innerHTML = `שגיאה בשליפת זמן`; }
  }
  ...
  const headerDiv = el("div", { id: "anipet-pro-header" }, [
    el("div", { style: { display: "flex", alignItems: "center", gap: "12px" } }, [
      ...
      el("div", {}, [ el("div", { textContent: "AniPet PRO · מערכת רכש", style: { fontSize: "16px", fontWeight: "700" } }), el("div", { textContent: "V2.14.6 - Cross-Project Sync & LocalStorage Fallback", style: { fontSize: "12px", color: "#94a3b8" } }) ])
    ]),
          ...
          Promise.all([
              ...
              if (dedupedCusts.length > 0) {
                  ...
                  dedupedCusts.forEach(c => {
                      ...
                      let addrHtml = addrObj ? `📍 <b>${addrObj.c_city}</b>${addrObj.c_address ? ' - ' + addrObj.c_address : ''}` : '';
                      let li = el("li", { innerHTML: `<div style="font-size:14px; font-weight:bold; color:#0f172a;">${c.name} <span style="color:#64748b; font-weight:normal;">(${dispPhone})</span></div>${addrHtml ? `<div style="font-size:12px; color:#64748b;">${addrHtml}</div>` : ''}` });
  ...
  controlsRow2.appendChild(el("div", { className: "ani-input-group" }, [ el("label", { className: "ani-label has-tooltip", textContent: "סדר תצוגה", 'data-tooltip': "<b>איך עובד המיון?</b><br>• <b>SOS:</b> שקלול של תאריך קרוב, אמינות הלקוח וכמות המוצר.<br>• <b>סכנת נטישה:</b> מיון לפי מי שנמצא באיחור הגדול ביותר.<br>• <b>קרוב להיום:</b> מיון פשוט לפי תאריך היעד הקרוב ביותר.<br>• <b>כמות יחידות/לקוחות:</b> מיון לפי נפח כדי לאתר מוצרים חמים." }), selSort ]));
  ...
  async function fetchHistoryDirect(p9, categoryKey, cName, infoBtn) {
      ...
      const originalInner = infoBtn.innerHTML; infoBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin" style="color:#64748b"></i>';
      try {
          ...
          const histRows = histData.map(d => {
              ...
              return [ fmtDate(d.order_date || d.o_date), d.item_name || 'מוצר ללא שם', badge(d.consumption_category ? t(d.consumption_category) : (d.category_name ? t(d.category_name) : "כללי"), "gray"), qty, weightStr ];
          });
      ...
      } catch(err) { alert("שגיאה בתקשורת מול השרת: " + err); } finally { infoBtn.innerHTML = originalInner; }
  }
  ...
  async function fetchAllData() {
      ...
      try {
          ...
          state.allRawData = rawData.map(r => {
             ...
             return { ...r, customer_name: n ? n.c_name : 'לקוח לא ידוע', customer_city: n ? n.c_city : '', customer_addr: n ? n.c_address : '' };
          });

          const allSkus = [...new Set(state.allRawData.map(s => s.recommend_sku).filter(Boolean))];
          await fetchMetaForSkus(allSkus); renderCurrentView();
  ...
  function renderMainList() {
    ...
    let filteredRows = state.allRawData.filter(r => {
        const info = parseCategoryInfo(r.category_key); const sSup = state.supplierMap[r.recommend_sku] || state.supplierMap[r.item_name]; const relStatus = getReliabilityCategory(r.cv_percentage);
    ...
    filteredRows.forEach(r => {
        if (!skuMap[r.recommend_sku]) skuMap[r.recommend_sku] = { recommend_sku: r.recommend_sku, product_name: r.item_name, category_key: r.category_key, total_customers: 0, late_customers: 0, future_customers: 0, nearest_due_date: '2099-01-01', total_expected_qty: 0, total_expected_weight: 0, reliable_customers: 0, customers: [] };
        ...
        if (getReliabilityCategory(r.cv_percentage) !== 'erratic' && getReliabilityCategory(r.cv_percentage) !== 'sparse') s.reliable_customers++;
        s.customers.push(r);
    });

    let aggregatedSkus = Object.values(skuMap).filter(s => {
        if ((state.activePreset === 'buyer_action' || state.sortBy === 'smart') && !state.filterCustomerRel.includes('swiss')) { if (s.total_expected_weight < 3 && s.reliable_customers === 0) return false; }
        return true;
    });
    ...
    const sortedSkus = aggregatedSkus.sort((a, b) => {
      const dA = new Date(a.nearest_due_date).setHours(0,0,0,0); const dB = new Date(b.nearest_due_date).setHours(0,0,0,0); const today = new Date().setHours(0,0,0,0);
      if (state.sortBy === 'smart') {
          const aRel = a.reliable_customers > 0 ? 1 : 0; const bRel = b.reliable_customers > 0 ? 1 : 0;
  ...
  function buildSkuCard(s, isSearchMode = false) {
      ...
      let inWindowText = s.future_customers||0; let lateText = s.late_customers||0;

      if (!isSearchMode) {
          badgeList.push(el("span", { className: "ani-badge badge-green has-tooltip", 'data-tooltip': "צפי עתידי", innerHTML: `<i class="fa-regular fa-calendar-check" style="margin-left:4px;"></i>צפי עתידי: <b>${inWindowText}</b>` }));
      ...
      const textArea = el("div", { style: { display: "flex", flexDirection: "column", gap: "6px" } }, [
        el("div", { style: { display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" } }, [
          ...
          asNode(el("span", { className: "ani-badge badge-gray has-tooltip", 'data-tooltip': 'מק"ט', innerHTML: `<i class="fa-light fa-barcode" style="margin-left:4px;"></i>${s.recommend_sku}` })),
  ...
  async function renderCustomersForSku(users, containerElement) {
      try {
          ...
          for (const u of users) {
              ...
              let anomalyBadge = badge("יציב", "gray", "ממוצע תואם להזמנה"); let relBadge = getReliabilityBadge(u.cv_percentage);
              ...
              const exactDateStr = fmtDate(u.next_expected_date); const rangeStr = getDateRangeStr(u.next_expected_date, u.cv_percentage);
              const dateNodeInner = el("div", {style:{display:"flex", flexDirection:"column", alignItems:"center", whiteSpace:"nowrap", lineHeight:"1.3"}});
              dateNodeInner.appendChild(el("span", {className:"has-tooltip", "data-tooltip": "תאריך סיום מלאי (מנוע V4)", style: { color: "#0f172a", fontWeight: "600" }, textContent: exactDateStr}));
              ...
              let addrHtml = u.customer_city ? `<b>${u.customer_city}</b>${u.customer_addr ? ' - ' + u.customer_addr : ''}` : (u.customer_addr || "לא הוזנה כתובת");
              custDetails.appendChild(el("div", {style:{fontSize:"11px", color:"#64748b"}, innerHTML: addrHtml}));
              ...
              histDetails.appendChild(el("div", {style:{fontSize:"11px", color:"#64748b"}, className: "has-tooltip", "data-tooltip": "סה״כ הזמנות בקטגוריה זו אי פעם"}, [`סה"כ ${uOrders.length} הזמנות`]));
              ...
              infoBtn.onclick = (e) => { e.stopPropagation(); fetchHistoryDirect(p9, u.category_key, cName, infoBtn); };
  ...
  function renderCustomerList() {
      ...
      let filtered = Object.values(cMap).filter(c => {
          ...
          for(const cat of c.categories) {
              const info = parseCategoryInfo(cat.category_key); const relStatus = getReliabilityCategory(cat.cv_percentage);
      ...
      filtered.forEach(c => { mainList.appendChild(buildCustomerCard(c.phone, c.name, c.city, c.addr, c.categories, false)); });
  }
  ...
  function buildCustomerCard(phone, name, city, addr, categories, autoOpen = false) {
      ...
      let addrHtml = city ? `<b>${city}</b>${addr ? ' - ' + addr : ''}` : (addr || "לא הוזנה כתובת");
  ...
  async function loadFeedbackDataAndRender() {
      ...
      try {
          const res = await gmFetch(`${SUPABASE_URL}/rest/v1/rpc/buyer_predictions_feedback_loop`, { method: "POST", headers: sbHeaders(), body: JSON.stringify({ p_days_back: state.feedbackDaysBack }) });
  ...
  async function searchProductBySku(sku, name, catName) {
      ...
      try {
          ...
          users = users.map(u => {
              let paddedP = (!u.customer_phone.startsWith('0') && (u.customer_phone.length === 8 || u.customer_phone.length === 9)) ? '0' + u.customer_phone : u.customer_phone; let n = namesMap[paddedP] || namesMap[u.customer_phone]; return {...u, customer_name: n?.c_name, customer_city: n?.c_city, customer_addr: n?.c_address};
          });
  ...
  async function searchCustomerByPhone(p9) {
      ...
      const namesRes = await gmFetch(`${SUPABASE_URL}/rest/v1/rpc/buyer_sku_shortages_raw_names`, { method: "POST", headers: sbHeaders(), body: JSON.stringify({ p_phones: [p9, paddedP] }) });
      let cName = "לקוח לא ידוע", cCity = "", cAddr = "";
      if (namesRes.ok) { const nd = tryParse(namesRes.responseText); if (nd && nd.length) { cName = nd[0].c_name; cCity = nd[0].c_city; cAddr = nd[0].c_address; } }

      gmFetch(`${SUPABASE_URL}/rest/v1/mv_forecast_engine_v4?customer_phone=eq.${p9}`, { headers: sbHeaders() }).then(async res => {
          if(!res.ok) { setStatus("שגיאה בשליפת לקוח", true); return; }
          const data = tryParse(res.responseText); mainList.innerHTML = "";
          ...
          setStatus(`מציג פרופיל מלא עבור: ${cName}`); mainList.appendChild(buildCustomerCard(p9, cName, cCity, cAddr, data.map(d => ({...d, customer_name: cName, customer_city: cCity, customer_addr: cAddr})), true));
      });
  }
