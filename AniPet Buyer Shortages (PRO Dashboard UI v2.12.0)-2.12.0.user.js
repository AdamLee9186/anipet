// ==UserScript==
// @name         AniPet Buyer Shortages (PRO Dashboard UI v2.12.0)
// @namespace    anipet.buyer
// @version      2.12.0
// @description  Event-Driven Architecture: Triggers SimilarOrders.js modal via CustomEvent.
...
(() => {
  ...
  const css = `
    ...
    /* WhatsApp, Phone & Search Link CSS */
    .whatsapp-button, .phone-link-button, .history-search-button,
    .whatsapp-button i, .phone-link-button i, .history-search-button i { color: #3699ff !important; }

    .whatsapp-button, .phone-link-button, .history-search-button { margin-right: 8px; font-size: 1.6em; line-height: 30px; height: 30px; text-decoration: none; vertical-align: middle; transition: color 0.2s ease-in-out; display: inline-flex; align-items: center; justify-content: center; cursor: pointer; }

    .whatsapp-button:hover, .phone-link-button:hover, .history-search-button:hover,
    .whatsapp-button:hover i, .phone-link-button:hover i, .history-search-button:hover i { color: #0073e9 !important; }

    .whatsapp-injected { display: inline-flex; align-items: center; direction: ltr; }
  `;
  ...
  // ✨ NEW: כפתורים שיורים אירועים החוצה במקום לנהל את המודאל בעצמם
  function getPhoneNode(phone, name) {
      ...
      const waBtn = el("a", { href: `https://wa.me/${waDigits}`, target: "whatsapp_window", className: "whatsapp-button has-tooltip", 'data-tooltip': waTooltip });
      waBtn.innerHTML = '<i class="fa-brands fa-whatsapp"></i>';
      waBtn.onclick = (e) => e.stopPropagation();

      const telBtn = el("a", { href: `tel:${digits}`, className: "phone-link-button has-tooltip", 'data-tooltip': telTooltip });
      telBtn.innerHTML = '<i class="fa-light fa-mobile"></i>';
      telBtn.onclick = (e) => e.stopPropagation();

      const histBtn = el("a", { className: "history-search-button has-tooltip", 'data-tooltip': histTooltip });
      histBtn.innerHTML = '<i class="fa-solid fa-magnifying-glass"></i>';
      histBtn.onclick = (e) => {
          e.stopPropagation();
          // 🌟 פריצת הדרך: הפעלת מודאל ה-SimilarOrders המקורי דרך Event
          document.dispatchEvent(new CustomEvent('OpenSimilarOrdersByPhone', { detail: { phone: dispPhone } }));
      };

      return el("span", { className: "whatsapp-injected" }, [
          el("span", { textContent: dispPhone, style: { fontWeight: "600", color: "#0f172a", marginRight: "8px" } }),
          waBtn,
          telBtn,
          histBtn
      ]);
  ...
  const headerDiv = el("div", { id: "anipet-pro-header" }, [
    el("div", { style: { display: "flex", alignItems: "center", gap: "12px" } }, [
      ...
      el("div", {}, [
        ...
        el("div", { textContent: "V2.12.0 - אינטגרציית SimilarOrders (Event Bridge)", style: { fontSize: "12px", color: "#94a3b8" } })
      ])
    ]),
