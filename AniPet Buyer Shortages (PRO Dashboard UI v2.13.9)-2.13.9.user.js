// ==UserScript==
// @name         AniPet Buyer Shortages (PRO Dashboard UI v2.13.9)
// @namespace    anipet.buyer
// @version      2.13.9
// @description  Fixed Leading Zero bug for missing Customer Names + Notes Integration.
...
(() => {
  ...
  const css = `
    ...
    .ani-action-btn:hover, .ani-action-btn:hover i {
        color: #0056b3 !important;
    }

    .ani-action-btn.lwcn-btn {
        border: 1px solid rgba(17,24,39,0.15) !important;
        background: linear-gradient(180deg, #ffffff, #f7f7f9) !important;
    ...
    .ani-action-btn.lwcn-btn.lwcn-has-note {
        border-color: rgba(249,115,22,0.55) !important;
        background: rgba(255,189,46,0.22) !important;
        box-shadow: 0 0 0 2px rgba(249,115,22,0.16) !important;
    }
    .ani-action-btn.lwcn-btn.lwcn-has-note i { color: #F97316 !important; }

    @keyframes lwcnPulsePro {
        0%   { transform: scale(1);     box-shadow: 0 0 0 0 rgba(245,158,11,0.38); }
        50%  { transform: scale(1.035); box-shadow: 0 0 0 5px rgba(245,158,11,0.10); }
        100% { transform: scale(1);     box-shadow: 0 0 0 0 rgba(245,158,11,0.00); }
    }
    .ani-action-btn.lwcn-btn.lwcn-blink {
        animation: lwcnPulsePro 1.8s ease-in-out infinite !important;
        border-color: rgba(245,158,11,0.75) !important;
    }

    .ani-thumb-img {
        transition: opacity 0.2s ease, transform 0.1s ease; cursor: pointer; }
    .ani-thumb-img:hover {
  ...
  function parseCategoryInfo(catKey) {
      ...
      if (key.includes('dog')) animal = 'dog'; else if (key.includes('cat')) animal = 'cat';
      else if (key.includes('bird')) animal = 'bird'; else if (key.includes('rodent')) animal = 'rodent';
      if (key.includes('litter') || key.includes('sandy')) type = 'litter';
      else if (key.includes('treats')) type = 'treats'; else if (key.includes('wet')) type = 'wet'; else if (key.includes('food')) type = 'dry';
      return { animal, type };
  }
  ...
  function getFullSizeImageUrl(thumbnailUrl) {
      try {
          ...
          if (thumbnailUrl.includes('just4pet.co.il')) {
              ...
              const filenameWithQuery = parts.pop(); const filenameParts = filenameWithQuery.split('?');
  ...
  async function copyImageToClipboard(imgElement, rawUrl) {
      ...
      setStatus("מוריד תמונה מלאה ללוח...");
      try {
          ...
          setStatus("התמונה הועתקה ללוח בהצלחה! ✅");
          setTimeout(() => setStatus("מוכן"), 3000);
      } catch (err) {
          setStatus("לא ניתן להעתיק תמונה ישירות. פותח בחלון חדש...");
  ...
  function getPhoneNode(phone, name) {
      ...
      let normPhone = digits;
      if (normPhone.length === 9 || normPhone.length === 8) normPhone = "0" + normPhone;

      const hasNote = state.notesIndex && state.notesIndex.has(normPhone);
      const noteBtnClass = hasNote ? "ani-action-btn lwcn-btn lwcn-has-note lwcn-blink has-tooltip" : "ani-action-btn lwcn-btn has-tooltip";
      const noteBtn = el("span", { className: noteBtnClass, 'data-tooltip': "הערות לקוח (נפתח מול Customer Notes)" });
      noteBtn.innerHTML = '<i class="fa-regular fa-note-sticky"></i>';
      noteBtn.onclick = (e) => {
          e.stopPropagation();
          document.dispatchEvent(new CustomEvent('OpenLionwheelNote', { detail: { phone: dispPhone, name: name, buttonElement: noteBtn } }));
      };

      return el("span", { className: "ani-actions-container" }, [
          noteBtn, histBtn, telBtn, waBtn,
          el("span", { textContent: dispPhone, style: { fontWeight: "600", color: "#0f172a" } })
      ]);
  }
  ...
  const headerDiv = el("div", { id: "anipet-pro-header" }, [
    el("div", { style: { display: "flex", alignItems: "center", gap: "12px" } }, [
      ...
      el("div", {}, [
        ...
        el("div", { textContent: "V2.13.9 - Name Bug Fix + Notes Ready", style: { fontSize: "12px", color: "#94a3b8" } })
      ])
    ]),
      ...
      searchTimeout = setTimeout(() => {
          ...
          Promise.all([
              ...
              if (dedupedCusts.length > 0) {
                  ...
                  // ✨ תיקון חיפוש השמות (Leading Zero) לחלון החיפוש הכללי
                  const rawPhones = dedupedCusts.map(c => c.phone);
                  let fetchPhones = [];
                  rawPhones.forEach(p => {
                      fetchPhones.push(p);
                      if (!p.startsWith('0') && (p.length === 8 || p.length === 9)) fetchPhones.push('0' + p);
                  });
                  ...
                  dedupedCusts.forEach(c => {
                      ...
                      let paddedP = (!c.phone.startsWith('0') && (c.phone.length === 8 || c.phone.length === 9)) ? '0' + c.phone : c.phone;
                      let addr = namesMap[paddedP] || namesMap[c.phone] || '';
  ...
  async function fetchAllData() {
      ...
      try {
          ...
          const url = `${SUPABASE_URL}/rest/v1/mv_forecast_engine_v4?next_expected_date=gte.${gStr}&next_expected_date=lte.${wStr}&limit=3000`;
          const res = await gmFetch(url, { headers: sbHeaders() });
          if(!res.ok) throw new Error("API responded with " + res.status);
          let rawData = tryParse(res.responseText) || [];

          try {
              const notesUrl = `${SUPABASE_URL}/rest/v1/customer_notes?select=customer_key`;
              const nRes = await gmFetch(notesUrl, { headers: sbHeaders() });
              if (nRes.ok) {
                  const nData = tryParse(nRes.responseText) || [];
                  state.notesIndex = new Set();
                  nData.forEach(n => { if (n.customer_key) state.notesIndex.add(n.customer_key); });
              }
          } catch(e) {}

          // ✨ תיקון חיפוש השמות (הוספת אפס מוביל)
          const rawPhones = rawData.map(c => c.customer_phone).filter(Boolean);
          let fetchPhones = [];
          rawPhones.forEach(p => {
              fetchPhones.push(p);
              if (!p.startsWith('0') && (p.length === 8 || p.length === 9)) {
                  fetchPhones.push('0' + p);
              }
          });
          const phonesArr = [...new Set(fetchPhones)];
          ...
          state.allRawData = rawData.map(r => {
             let p = r.customer_phone;
             let paddedP = (!p.startsWith('0') && (p.length === 8 || p.length === 9)) ? '0' + p : p;
  ...
  function buildSkuCard(s, isSearchMode = false) {
      ...
      const thumbnailEl = el("img", {
          ...
          className: "ani-thumb-img", title: "לחץ להעתקת התמונה המלאה ללוח",
  ...
  async function renderCustomersForSku(users, containerElement) {
      try {
          ...
          for (const u of users) {
              ...
              rawUOrders.forEach(o => {
                  let dStr = o.date.toISOString().split('T')[0];
                  if(!dMap[dStr]) dMap[dStr] = { date: new Date(dStr), qty: 0 };
                  dMap[dStr].qty += o.qty;
              });
                  ...
                  if (past.length > 0) {
                      ...
                      if (diffUnitsAbs >= 1) {
                          ...
                          if (lastQtyUnits < avgQtyUnitsRaw) {
                               anomalyBadge = badge(`▼ קנה פחות ב-${diffStr}`, "red", `הזמין כמות קטנה מהממוצע הרגיל שלו (ממוצע: ${avgQtyUnits} יח')`);
                          } else if (lastQtyUnits > avgQtyUnitsRaw) {
                              anomalyBadge = badge(`▲ קנה יותר ב-${diffStr}`, "green", `הזמין כמות גדולה מהממוצע הרגיל שלו (ממוצע: ${avgQtyUnits} יח')`);
                          }
                      }
                  }
              }
              ...
              let isDryFood = ['dog_food', 'cat_food'].includes(u.category_key);

              let logicTxt = isDryFood ? "שימוש בחישוב 90% לשקי מזון יבש (התרעה לפני סיום)" : "ספירת יחידות לפי קצב צריכה רגיל";
              ...
              let totalQtyDisplay = u.expected_qty ? `${Math.round(u.expected_qty)} יחידות` : '';
              let explainText = `<div style="font-size:14px; font-weight:bold; margin-bottom:8px; border-bottom:1px solid #475569; padding-bottom:4px;">איך הגענו לתאריך הזה? (מנוע V4)</div>`;
              explainText += `<div style="margin-bottom:4px;">📦 <b>קנייה אחרונה:</b> ${totalQtyDisplay}</div>`;
              explainText += `<div style="margin-bottom:4px;">⚙️ <b>שיטת חישוב:</b> ${logicTxt}</div>`;
              explainText += `<div style="margin-bottom:4px;">⏳ <b>לכמה ימים זה אמור להספיק?</b> ${u.dos_days} ימים</div>`;
              explainText += `<div style="margin-bottom:8px;">🛡️ <b>ימי חסד/הקדמה:</b> הוספו כ-${offsetDays} ימים (התאמה אישית להתנהגות הלקוח)</div>`;
              explainText += `<div style="background:rgba(255,255,255,0.1); padding:4px; border-radius:4px; font-weight:bold; color:#fde047;">תאריך יעד: ${fmtDate(u.next_expected_date)}</div>`;
  ...
  async function loadFeedbackDataAndRender() {
      ...
      try {
          ...
          // ✨ תיקון Leading Zero לחלון ההצלחות
  ...
  async function searchProductBySku(sku, name, catName) {
      ...
      try {
          const url = `${SUPABASE_URL}/rest/v1/mv_forecast_engine_v4?recommend_sku=eq.${sku}&limit=1000`;
          const res = await gmFetch(url, { headers: sbHeaders() });
          ...
          // ✨ התיקון גם בחיפוש המוצר (Leading Zero)
          const rawPhones = users.map(c => c.customer_phone).filter(Boolean);
          let fetchPhones = [];
          rawPhones.forEach(p => {
              fetchPhones.push(p);
              if (!p.startsWith('0') && (p.length === 8 || p.length === 9)) fetchPhones.push('0' + p);
          ...
          users = users.map(u => {
              let paddedP = (!u.customer_phone.startsWith('0') && (u.customer_phone.length === 8 || u.customer_phone.length === 9)) ? '0' + u.customer_phone : u.customer_phone;
              let n = namesMap[paddedP] || namesMap[u.customer_phone];
              return {...u, customer_name: n?.c_name, customer_addr: [n?.c_city, n?.c_address].filter(Boolean).join(" - ")};
          });
  ...
  async function searchCustomerByPhone(p9) {
      ...
      const url = `${SUPABASE_URL}/rest/v1/mv_forecast_engine_v4?customer_phone=eq.${p9}`;
