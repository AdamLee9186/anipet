// ==UserScript==
// @name         AniPet Buyer Shortages (PRO Dashboard UI v2.14.15)
...
(() => {
  ...
  const headerDiv = el("div", { id: "anipet-pro-header" }, [
    el("div", { style: { display: "flex", alignItems: "center", gap: "12px" } }, [
      ...
      el("div", {}, [ el("div", { textContent: "AniPet PRO · מערכת רכש", style: { fontSize: "16px", fontWeight: "700" } }), el("div", { textContent: "V2.14.15 - V6 Hybrid Engine (Fallback Aware)", style: { fontSize: "12px", color: "#94a3b8" } }) ])
    ]),
  ...
  async function fetchHistoryDirect(p9, categoryKey, cName, triggerNode = null) {
      ...
      try {
          const url = `${SUPABASE_URL}/rest/v1/v_orders_items_norm_api?customer_key=ilike.*${p9}*&consumption_category=eq.${categoryKey}&order=order_date.desc&limit=15`;
  ...
  async function fetchAllData() {
      ...
      try {
          ...
          const res = await gmFetch(`${SUPABASE_URL}/rest/v1/v_forecast_engine_v4_api?next_expected_date=gte.${gStr}&next_expected_date=lte.${wStr}&limit=3000`, { headers: sbHeaders() });
  ...
  async function searchProductBySku(sku, name, catName) {
      ...
      try {
          const res = await gmFetch(`${SUPABASE_URL}/rest/v1/v_forecast_engine_v4_api?recommend_sku=eq.${sku}&limit=1000`, { headers: sbHeaders() });
  ...
  async function searchCustomerByPhone(p9) {
      ...
      try {
          ...
          const [namesRes, dataRes] = await Promise.all([
              ...
              gmFetch(`${SUPABASE_URL}/rest/v1/v_forecast_engine_v4_api?or=(customer_phone.eq.${p9},customer_phone.eq.${paddedP})`, { headers: sbHeaders() })
          ]);
