...
function showGalleryOverlay(items, startIndex){
  ...
  if (url) window.open(url, '_blank', 'noopener');
        ...
        // === NEW: Try to auto-apply the "בטיפול" filter before building the table ===
        ...
        async function applyTreatmentFilterIfExists(){
            try{
                ...
                // bring into view & simulate a real click (Tampermonkey-safe, no 'view' field)
                ...
                ['pointerdown', 'pointerup', 'mousedown', 'mouseup', 'click'].forEach(type => {
                    try {
                        ...
                        try { badge.dispatchEvent(new MouseEvent(type, evtInit)); } catch (_) {}
                    }
                });
