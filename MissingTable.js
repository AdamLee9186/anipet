    ...
    */
    ...
    (function () {
        ...
        const CONFIG = Object.assign({
            ...
            SHARED_SUPPLIER_INDEX_TO_SESSION: false, // try to persist compact supplier index in sessionStorage
        ...
        // Shared cross-script namespace (in-page only)
        ...
        function lwPublishSharedSupplierIndex(indexObj, { persistSession = false } = {}) {
            ...
            try {
                const payload = JSON.stringify(indexObj);
                sessionStorage.setItem(LW_SHARED_SESSION_SUPPLIER_KEY, payload);
            } catch (e) {
        ...
        function lwBuildCompactSupplierIndex(data) {
            const suppliers = (data && data.metadata && Array.isArray(data.metadata.suppliers_list))
                ? data.metadata.suppliers_list
                : [];

            const supplierToId = new Map();
            suppliers.forEach((s, i) => supplierToId.set(String(s), i));

            const byBarcode = Object.create(null); // barcode -> supplierId
            const bySku = Object.create(null);     // sku -> supplierId
        ...
        const MT_TASK_CACHE_TTL_MS = 10 * 60 * 1000; // 10 minutes (bulk_update hooks invalidate instantly; TTL is a safety net)
        ...
        // ETag/Last-Modified aware fetch for the catalog CSV - Safe Version
        async function fetchWithValidators(url) {
            ...
            try {
                cachedBody = localStorage.getItem(key("body"));
                cachedETag = localStorage.getItem(key("etag"));
                cachedLM = localStorage.getItem(key("lm"));
        ...
        // Cache policy (balanced performance vs accuracy):
        // - Live tasks: TTL 2 hours (fresh enough for a few daily updates)
        // - Closed tasks: TTL 24 hours
        // - SWR: for live tasks, if cache age > 45 minutes, use cache now (fast)
        //        but refresh silently in the background (updates cache for next run)
        const TASK_CACHE_TTL_LIVE_MS = 2 * 60 * 60 * 1000;      // 2 hours
        const TASK_CACHE_TTL_CLOSED_MS = 24 * 60 * 60 * 1000;   // 24 hours
        const TASK_CACHE_SWR_REFRESH_AFTER_MS = 45 * 60 * 1000; // 45 minutes
        ...
        function safeSaveTaskCache() {
            ...
            try {
                ...
                // זיהוי שגיאת מכסה מלאה
                if (err && (err.name === 'QuotaExceededError' || err.code === 22 || err.code === 1014)) {
                    console.warn('[MissingTable] ⚠️ Storage full during Task Save. Nuking Catalog Cache to make space...');

                    // 1. נקה באגרסיביות את הקאש הגדול (קטלוג CSV ו-JSON)
                    try {
                        Object.keys(localStorage).forEach(k => {
                            if (k.includes('catalog_csv') || k.includes('products_optimized') || k.includes(CONFIG.CACHE_NS)) {
                                localStorage.removeItem(k);
                            }
                        });
                        // נקה גם session storage מיותר אם יש
                        Object.keys(sessionStorage).forEach(k => {
                             if (k !== TASK_CACHE_KEY && (k.includes('catalog') || k.includes('cache'))) {
                                 sessionStorage.removeItem(k);
                             }
                        });
                    } catch(e) { console.error('Cleanup failed', e); }

                    // 2. נסה לשמור שוב את המשימות (זה החלק הקריטי כדי שהמחיקה תישמר)
                    try {
                        sessionStorage.setItem(TASK_CACHE_KEY, JSON.stringify(taskCache));
                        console.log('✅ Task cache saved successfully after emergency cleanup.');
                    } catch (e2) {
                        console.error('[MissingTable] ❌ Storage STILL full. Disabling task cache for this session to prevent errors.');
                        disableTaskCache = true;
                    }
                } else {
                    console.error('[MissingTable] Cache save error:', err);
                }
            }
        }
        ...
        function isClosedTaskStatus(status) {
            ...
            // Consider these immutable/closed for caching purposes
            return (
                s === 'PICKED' ||
                s === 'COMPLETED' ||
                s === 'CANCELED' ||
        ...
        function getTaskCacheTTLms(entry) {
            const status = entry?.status;
            return isClosedTaskStatus(status) ? TASK_CACHE_TTL_CLOSED_MS : TASK_CACHE_TTL_LIVE_MS;
        }

        // Returns { data, ageMs, needsBackgroundRefresh, status } or null (miss/expired)
        function readTaskCacheEntry(id, ver) {
            ...
            const ttl = getTaskCacheTTLms(c);

            // TTL expired -> invalidate entry (force fetch)
            if (ageMs > ttl) {
                if (CONFIG.DEBUG) {
                    console.log(`[Cache] Task ${id}: Cache expired (age: ${Math.round(ageMs/1000)}s > ${Math.round(ttl/1000)}s), invalidating`);
                }
                delete taskCache[id];
                saveTaskCache();
                return null;
            ...
            const isClosed = isClosedTaskStatus(c.status);
            const needsBackgroundRefresh = !isClosed && ageMs > TASK_CACHE_SWR_REFRESH_AFTER_MS;
        ...
        /**
         * Hard invalidate cache for a single task.
         *
         * Why needed:
         * - Even with a better row signature, Lionwheel often does NOT update the main table
         *   when quantities are edited inside the "pick items" modal.
         * - Therefore, the cache can remain "TTL OK" while the underlying order_item.items changed.
         *
         * This function is called by network hooks (fetch/XHR) when we detect order_items/bulk_update.
         */
        function invalidateTaskCache(taskId, reason = "manual") {
            try {
                const key = String(taskId || "").trim();
                if (!key) return false;

                const existed = Object.prototype.hasOwnProperty.call(taskCache, key);
                delete taskCache[key];
                safeSaveTaskCache();
                console.log(`[MissingTable] Cache invalidated for task ${key}${reason ? ` (${reason})` : ''}${existed ? '' : ' (not previously cached)'}`);
                return existed;
            } catch (err) {
                console.warn("[MissingTable] Failed to invalidate task cache", err);
                return false;
            }
        }

        /**
         * Extract task_id from Lionwheel bulk update responses.
         * Observed shapes:
         * - { task_id: "21793041", order_item: { items: {...} } }
         * - { task_id: 21793041, ... }
         */
        function extractTaskIdFromBulkUpdatePayload(payload) {
            if (!payload || typeof payload !== "object") return null;

            // Most common.
            if (payload.task_id != null) return String(payload.task_id);

            // Defensive fallbacks.
            if (payload.taskId != null) return String(payload.taskId);
            if (payload.order_item && (payload.order_item.task_id != null)) return String(payload.order_item.task_id);
            if (payload.orderItem && (payload.orderItem.task_id != null)) return String(payload.orderItem.task_id);

            return null;
        }

        /**
         * Extract task_id from request body (fetch init.body or XHR send body).
         * This is the PRIMARY source because the response usually doesn't include task_id.
         */
        function extractTaskIdFromBulkUpdateBody(body) {
            try {
                ...
                // XHR send(body) where body could be FormData
                if (typeof FormData !== 'undefined' && body instanceof FormData) {
                    const s = body.get('task_id') || body.get('taskId');
                    return s ? String(s).trim() : null;
                }

                if (obj && typeof obj === 'object') {
                    const tid = obj.task_id ?? obj.taskId ?? obj?.order_item?.task_id;
                    return tid ? String(tid).trim() : null;
                }
            } catch (e) {
                // Ignore parse errors; we'll fallback to other strategies
            }
            return null;
        }

        /**
         * Network hooks: detect quantity changes (order_items/bulk_update) and invalidate cache for that task.
         *
         * Important:
         * - Works even if the user later refreshes, because our cache is stored in sessionStorage.
         * - Logs a clear line so it is easy to verify in DevTools.
         */
        /* Fix: patch fetch/XHR in page context (unsafeWindow) so bulk_update invalidation logs actually fire */
        function installCacheInvalidationHooks() {
            try {
                // IMPORTANT:
                // In Tampermonkey, scripts often run in an isolated sandbox.
                // Lionwheel uses the page's fetch/XHR, so we must patch the PAGE window (unsafeWindow) when available.
                const W = (typeof unsafeWindow !== 'undefined' && unsafeWindow) ? unsafeWindow : window;

                if (W.__MT_CACHE_INVALIDATION_HOOKS_INSTALLED__) return;
                W.__MT_CACHE_INVALIDATION_HOOKS_INSTALLED__ = true;

                const isBulkUpdateUrl = (u) => {
                    try {
                        const s = String(u || "");
                        return /order_items\/bulk_update/i.test(s) || /bulk_update/i.test(s);
                    } catch {
                        return false;
                    }
                };
                ...
                // --- fetch hook ---
                if (typeof W.fetch === "function" && !W.__MT_FETCH_PATCHED__) {
                    W.__MT_FETCH_PATCHED__ = true;
                    const origFetch = W.fetch.bind(W);
                    W.fetch = async function patchedFetch(input, init) {
                        const url = (typeof input === "string") ? input : (input && input.url);
                        const method = (init && init.method) || (input && input.method) || "GET";

                        // NEW: invalidate from REQUEST body (works even if response has no task_id)
                        try {
                            if (url && /\/order_items\/bulk_update/i.test(url) && String(method).toUpperCase() === 'POST') {
                                const taskId = extractTaskIdFromBulkUpdateBody(init && init.body);
                                if (taskId) {
                                    invalidateTaskCache(taskId, 'bulk_update (request body)');
                                } else {
                                    console.warn('[MissingTable] bulk_update detected but task_id was not found in request body');
                                }
                            }
                        } catch (e) {
                            console.warn('[MissingTable] bulk_update fetch hook error:', e);
                        }

                        const resp = await origFetch(input, init);

                        // Fallback: try response (optional, usually doesn't have task_id)
                        try {
                            if (resp && resp.ok && isBulkUpdateUrl(url) && String(method).toUpperCase() !== "GET") {
                                ...
                                if (/json/i.test(ct)) {
                                    ...
                                    const taskId = extractTaskIdFromBulkUpdatePayload(data);
                                    if (taskId) invalidateTaskCache(taskId, "bulk_update:fetch:resp");
                                }
                            }
                        } catch (e) {
                            // Never break the app flow.
                            console.warn("[MissingTable] fetch invalidation hook error", e);
                        }
                ...
                // --- XHR hook (Lionwheel sometimes uses jQuery/Ajax) ---
                if (W.XMLHttpRequest && !W.__MT_XHR_PATCHED__) {
                    W.__MT_XHR_PATCHED__ = true;
                    ...
                    W.XMLHttpRequest.prototype.send = function (...args) {
                        try {
                            this.__mt_body = args && args.length ? args[0] : null; // NEW: save body for later inspection
                            this.addEventListener("load", () => {
                                try {
                                    const url = this.__mt_url;
                                    const method = this.__mt_method || "GET";
                                    if (!url || !/\/order_items\/bulk_update/i.test(url) || String(method).toUpperCase() !== 'POST') return;

                                    // NEW: try request body first (this is where task_id usually is)
                                    const taskId = extractTaskIdFromBulkUpdateBody(this.__mt_body);
                                    if (taskId) {
                                        invalidateTaskCache(taskId, 'bulk_update (request body)');
                                        return; // Success, no need to check response
                                    } else {
                                        console.warn('[MissingTable] bulk_update XHR detected but task_id was not found in request body');
                                    }
                                    ...
                                    const taskIdFromResp = extractTaskIdFromBulkUpdatePayload(data);
                                    if (taskIdFromResp) invalidateTaskCache(taskIdFromResp, "bulk_update:xhr:resp");
                                } catch (e) {
                                    // Swallow - never block.
                                }
                            });
                ...
                console.log('[MissingTable] Cache invalidation hooks installed (bulk_update) on page window:', W === window ? 'window' : 'unsafeWindow');
            } catch (err) {
                console.warn("[MissingTable] Failed to install cache invalidation hooks", err);
            }
        }

        // Important: install at load time (document-start) so we patch fetch/XHR
        // before Lionwheel captures references to them.
        if (/^(members|test)\.lionwheel\.com$/i.test(location.hostname)) {
            installCacheInvalidationHooks();
        }
        ...
        // Backwards-compatible wrapper (existing call sites may expect only HTML string)
        function readTaskFromCache(id, ver, opts) {
            opts = opts || {};
            const forceRefresh = !!opts.forceRefresh;
            if (forceRefresh) return null;
            const entry = readTaskCacheEntry(id, ver);
            return entry ? entry.data : null;
        }

        function writeTaskToCache(id, ver, data, status = 'UNKNOWN') {
            if (disableTaskCache) return;
            taskCache[id] = { ver, data, ts: Date.now(), status }; // ts for LRU, status for TTL rules
            saveTaskCache();
        }
...
function showGalleryOverlay(items, startIndex){
  ...
  if (url) window.open(url, '_blank', 'noopener');
        ...
        async function loadSupplierData() {
            if (supplierDataLoaded) return;

            // 1) Prefer shared supplier index if available
            const shared = lwGetSharedSupplierIndex();
            if (shared && shared.byBarcode && shared.suppliers) {
                supplierIndex = shared;
                supplierDataLoaded = true;
                return;
            }

            // 2) Optionally restore compact index from sessionStorage
            try {
                const raw = sessionStorage.getItem(LW_SHARED_SESSION_SUPPLIER_KEY);
                if (raw) {
                    const parsed = JSON.parse(raw);
                    if (parsed && parsed.byBarcode && parsed.suppliers) {
                        supplierIndex = parsed;
                        lwPublishSharedSupplierIndex(supplierIndex, { persistSession: false });
                        supplierDataLoaded = true;
                        return;
                    }
                }
            ...
            // 3) Fetch + build compact index (and optional byName fallback map)
            console.log("[MT] Loading supplier data (build compact index)...");
            try {
                const response = await fetchWithValidators(CONFIG.PRODUCTS_JSON_URL);
                const data = await response.json();

                if (!data || !Array.isArray(data.products)) {
                    console.warn("[MT] Supplier data format invalid.");
                    supplierDataLoaded = true;
                    return;
                }

                supplierIndex = lwBuildCompactSupplierIndex(data);
                lwPublishSharedSupplierIndex(supplierIndex, { persistSession: !!CONFIG.SHARED_SUPPLIER_INDEX_TO_SESSION });

                // Optional name fallback for THIS script only (not shared)
                supplierDataByNameMap.clear();
                for (const product of data.products) {
                    const name = String(product?.name || "").trim();
                    const supplier = String(product?.supplier || "").trim();
                    if (name && supplier) supplierDataByNameMap.set(name, supplier);
                }

                supplierDataLoaded = true;
                console.log("[MT] Supplier index ready (shared).");
            } catch (err) {
                console.error("[MT] Failed to load supplier data:", err);
                supplierDataLoaded = true;
            }
        }

        function mtLookupSupplier({ barcode, sku, name } = {}) {
            const b = (barcode ? String(barcode).trim() : '');
            const s = (sku ? String(sku).trim() : '');
            const n = (name ? String(name).trim() : '');

            const idx = supplierIndex || lwGetSharedSupplierIndex();
            if (idx && idx.suppliers) {
                if (b && idx.byBarcode && idx.byBarcode[b] !== undefined) {
                    return idx.suppliers[idx.byBarcode[b]] || null;
                }
                if (s && idx.bySku && idx.bySku[s] !== undefined) {
                    return idx.suppliers[idx.bySku[s]] || null;
                }
            }

            // fallback only for MissingTable itself
            if (n && supplierDataByNameMap && supplierDataByNameMap.has(n)) {
                return supplierDataByNameMap.get(n);
            }
            return null;
        }
        ...
        // Toolbar injection must wait: @run-at document-start runs before #kt_subheader exists
        const TOOLBAR_SELECTOR_PRIMARY =
          '#kt_subheader .d-flex.justify-content-sm-end.justify-content-start.align-items-stretch.flex-wrap';
        const TOOLBAR_SELECTOR_FALLBACK =
          '#kt_subheader .d-flex.justify-content-sm-end';

        const findTopToolbar = () =>
          document.querySelector(TOOLBAR_SELECTOR_PRIMARY) ||
          document.querySelector(TOOLBAR_SELECTOR_FALLBACK);

        const waitAndInjectButtons = () => {
          let tries = 0;
          const maxTries = 80;        // 80 * 250ms = 20s
          ...
          const timer = setInterval(() => {
            ...
            if (!toolbar) {
              if (tries >= maxTries) {
                clearInterval(timer);
                console.warn('⚠️ לא נמצא סרגל הכלים להוספת הכפתור (אחרי המתנה).');
              }
              return;
            }

            clearInterval(timer);
            injectMissingNegativeButtonsIntoToolbar(toolbar);
          }, intervalMs);
        };

        // Run after DOM is ready (still keeps hooks at document-start)
        if (document.readyState === 'loading') {
          document.addEventListener('DOMContentLoaded', waitAndInjectButtons, { once: true });
        } else {
          waitAndInjectButtons();
        }

        function injectMissingNegativeButtonsIntoToolbar(toolbar) {
        ...
        // Place in toolbar near the existing "פריטים" button if present
        const targetButton = toolbar.querySelector('.order-items-btn.order-items-summary-btn')
          || document.querySelector('.order-items-btn.order-items-summary-btn');
        ...
        } else if (rightmostButton && rightmostButton.parentElement) {
          rightmostButton.parentNode.insertBefore(btn, rightmostButton);
        } else {
          toolbar.appendChild(btn);
        }

        console.log('🎯 כפתורי "חוסרים" ו"נגטיב" נוספו לסרגל העליון');
        }
        ...
        // --- Task status extraction (used for cache TTL decisions) ---
        ...
        function getTaskOverallStatusFromDoc(doc, taskId) {
            try {
                let rawTaskStatusText = '';
                const taskStatusElement =
                    doc.querySelector('.editTaskStatus a[data-new-status]') ||
                    doc.querySelector('.editTaskStatus a[data-new-status][class*="badge"]') ||
                    doc.querySelector('.editTaskStatus a');

                if (taskStatusElement) {
                    const dataStatus = taskStatusElement.getAttribute('data-new-status');
                    if (dataStatus) rawTaskStatusText = dataStatus.trim();
                    else rawTaskStatusText = (taskStatusElement.textContent || '').trim();
                }

                const normalized = (rawTaskStatusText || '').trim();
                if (!normalized) return 'UNKNOWN';

                const mapped =
                    TASK_STATUS_MAPPING[normalized] ||
                    TASK_STATUS_MAPPING[normalized.toUpperCase()] ||
                    TASK_STATUS_MAPPING[normalized.toLowerCase()] ||
                    normalized.toUpperCase();

                return mapped || 'UNKNOWN';
            } catch (e) {
                if (CONFIG.DEBUG) console.warn(`[Cache] Task ${taskId}: failed to extract status`, e);
                return 'UNKNOWN';
            }
        }

        function extractTaskOverallStatusFromHtml(taskId, html) {
            try {
                const doc = new DOMParser().parseFromString(html, 'text/html');
                return getTaskOverallStatusFromDoc(doc, taskId);
            } catch (e) {
                return 'UNKNOWN';
            }
        }

        // Silent SWR refresh: update cache without blocking UI / without rerender
        function refreshTasksInBackground(taskIds, csrfToken, { maxConcurrent = 2 } = {}) {
            if (!taskIds || taskIds.length === 0) return;
            if (!csrfToken) return;

            const unique = Array.from(new Set(taskIds));
            const limitBg = pLimit(Math.max(1, Math.min(6, maxConcurrent)));

            if (CONFIG.DEBUG) {
                console.log(`[SWR] Background refresh for ${unique.length} live task(s) (concurrency=${Math.max(1, Math.min(6, maxConcurrent))})`);
            }

            // Fire-and-forget (do not await)
            (async () => {
                const jobs = unique.map(taskId => limitBg(async () => {
                    try {
                        const ver = getUpdatedAtFromRow(taskId);
                        if (!ver) return;
                        const { html } = await fetchTaskPanelView(taskId, csrfToken, 0);
                        const status = extractTaskOverallStatusFromHtml(taskId, html);
                        writeTaskToCache(taskId, ver, html, status);
                        if (CONFIG.DEBUG) console.log(`[SWR] Refreshed cache for Task ${taskId} (status=${status})`);
                    } catch (e) {
                        if (CONFIG.DEBUG) console.warn(`[SWR] Failed refreshing Task ${taskId}`, e);
                    }
                }));
        ...
        async function startMissingTable(forceRefresh = false) {
            ...
            try {
                ...
                // Cache check before fetch (with TTL + SWR)
                const tasksToProcess = [];
                const backgroundRefreshIds = new Set();

                for (const id of taskIds) {
                    const ver = getUpdatedAtFromRow(id);
                    const cachedEntry = readTaskCacheEntry(id, ver);
                    ...
                    if (forceRefresh || !cachedEntry) {
                        ...
                        if (CONFIG.DEBUG) {
                            console.log(`📦 Cache hit for Task ${id} (age=${Math.round(cachedEntry.ageMs/1000)}s, status=${cachedEntry.status})`);
                        }
                        ...
                        // SWR: refresh quietly for live tasks that are getting old
                        if (cachedEntry.needsBackgroundRefresh) {
                ...
                for (let i = 0; i < tasksToProcess.length; i++) {
                    ...
                    jobs.push(taskScheduler(async () => {
                        ...
                        try {
                            ...
                            const ver = getUpdatedAtFromRow(id);
                            const status = extractTaskOverallStatusFromHtml(taskId, html);
                            writeTaskToCache(id, ver, html, status);
                            fetchedRawItems.push({ html, id: taskId });
                        } catch (err) {
                ...
                // Kick off silent background refresh (SWR) without blocking UI
                if (backgroundRefreshIds.size > 0) {
                    const ctrl = { maxConcurrent: 2 };
                    void refreshTasksInBackground(Array.from(backgroundRefreshIds), csrfToken, ctrl);
                }
                ...
                responses.forEach(({ html, id: taskId }) => {
                    ...
                    const taskOverallStatus = getTaskOverallStatusFromDoc(doc, taskId);
                    if (CONFIG.DEBUG) console.log(`📋 Task ${taskId} overall status: ${taskOverallStatus}`);
                    ...
                    rows.forEach((row, index) => {
                        ...
                        // --- איתור ספק ---
                        let supplier = mtLookupSupplier({ barcode, sku: makt, name }) || 'ספק כללי';
